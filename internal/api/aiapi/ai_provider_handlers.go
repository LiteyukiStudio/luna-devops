...
func (h *Handler) GetAIProviderConfigInternal(ctx *gin.Context) {
	...
	values := h.configValues(aiProviderConfigKeys)
	var secretConfig model.AppConfig
	_ = h.dbFor(ctx).First(&secretConfig, "key = ?", "ai.provider.api_key").Error
	apiKey := h.resolveSecret(ctx.Request.Context(), secretConfig.Value)
	...
	ctx.JSON(http.StatusOK, gin.H{
		...
		"provider": gin.H{
			"baseUrl":                baseURL,
			"apiKey":                 apiKey,
			"providerCompatibility":  aiProviderSelectConfig(values, "ai.provider.compatibility", h.configDefinitionByKey),
			"promptCacheKeyMode":     aiProviderSelectConfig(values, "ai.provider.prompt_cache_key_mode", h.configDefinitionByKey),
		...
		"runtime":     aiProviderRuntimeConfig(values, h.configDefinitionByKey),
		"toolCatalog": toolCatalog,
	})
}

func aiProviderSelectConfig(values map[string]string, key string, definitionForKey func(string) *ConfigDefinition) string {
...
func aiProviderRuntimeConfig(values map[string]string, definitionForKey func(string) *ConfigDefinition) gin.H {
	return gin.H{
		"providerTimeoutMs":   aiBoundedIntegerConfig(values, "ai.runtime.provider_timeout_seconds", definitionForKey) * 1000,
		"maxRequestRetries":   aiBoundedIntegerConfig(values, "ai.runtime.max_request_retries", definitionForKey),
		"runTimeoutMs":        aiBoundedIntegerConfig(values, "ai.runtime.run_timeout_seconds", definitionForKey) * 1000,
		"agentConcurrentRuns": aiBoundedIntegerConfig(values, "ai.runtime.agent_concurrent_runs", definitionForKey),
		"userConcurrentRuns":  aiBoundedIntegerConfig(values, "ai.quota.user_concurrent_runs", definitionForKey),
	}
}

func aiBoundedIntegerConfig(values map[string]string, key string, definitionForKey func(string) *ConfigDefinition) int {
	bounds, ok := aiIntegerConfigBounds[key]
	if !ok {
		panic("missing AI integer configuration bounds for " + key)
	}
	return aiRuntimeIntegerInRange(values, key, aiDefaultIntegerConfig(key, definitionForKey), bounds[0], bounds[1])
}

func aiDefaultIntegerConfig(key string, definitionForKey func(string) *ConfigDefinition) int {
	definition := definitionForKey(key)
	if definition == nil {
...
func (h *Handler) TestAIProviderConnection(ctx *gin.Context) {
	user, ok := h.currentUser(ctx)
	if !ok {
		return
	}
	if h.aiUnavailableReason() != "" || h.host.AIAgent() == nil {
	...
	response, err := h.host.AIAgent().Do(ctx.Request.Context(), actor, aiagent.Request{
