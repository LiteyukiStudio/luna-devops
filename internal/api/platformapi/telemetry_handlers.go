...
func (h *Handler) RelayBrowserTraces(ctx *gin.Context) {
	...
	allowed, err := h.allowRate(ctx.Request.Context(), "browser_telemetry:"+user.ID, browserTelemetryRateLimit, time.Minute)
	if err != nil {
...
func otlpRelayHeaders(cfg sharedconfig.APIConfig) map[string]string {
