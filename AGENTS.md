...
- `docs/docs/{zh,en}` 是面向 Luna DevOps 使用者和部署管理员的公开文档；`notes/` 用于内部方案、研发记录、复盘和历史决策；`AGENTS.md` 用于开发约束。不同受众的内容不得混写。
...
- 公开文档默认不写 CI/发布门禁、覆盖率统计、内部仓库关系、分支策略、源码协作方式、实现架构、迁移过程、历史决策、内部待办或发版前剩余事项。这些内容应写入 `notes/`、`TODO.md`、代码注释或仓库内部开发规范。
