...
import { getAIUIActionTargetPath, parseAIOptionActions } from './actions'
...
export function AINavigationEvent({ block, onAction }: { block: ToolCallBlock, onAction: (action: AIUIAction) => Promise<boolean> }) {
  ...
  const action = parseAIOptionActions(block.uiActions).find(candidate => candidate.type === 'navigate')
  ...
  return (
    <button
      aria-label={t('aiAssistant.navigation.open', { target })}
    ...
    >
      ...
      <span className="truncate">{t('aiAssistant.navigation.ready', { target })}</span>
    </button>
  )
}
