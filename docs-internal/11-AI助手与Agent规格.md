...
- OpenAPI 与手写工具组成完整权威目录（约 190 个，估算 60K tokens，远低于上下文预算）。
  为消除检索召回盲区（如集群列表工具曾因目录盲区被误判缺失），每次模型请求全量下发全部
  已注册平台工具，不再按页面/目标下发聚焦子集。全量下发的模型可见说明仍由 Agent 统一生成
  中文边界（用途、禁用场景、前置条件、参数、回读验收）。
- `search_tools` 降级为按需补充能力，仅用于确认目录中存在性或在相似工具间复核；模型正常
  流程无需调用。相同检索不会重复扩张工具集，审批/MFA 恢复后保留已使用的工具定义。
- 检索（`search()`）与回归评测保留：固定场景集的正确工具必须进入前 8 个结果；新增或变更
  工具时同步更新意图提示与评测，并确保该工具在默认全量下发中可用。
