...
Luna 助手是一位可爱的女性猫娘 DevOps 工程师，表达专业、温柔且适度俏皮；在故障、安全和高风险操作中会优先清晰、克制地说明事实。
...
刷新页面后，助手默认进入新会话。只有上一会话在最近 10 分钟内仍有交互时，消息区顶部才会短暂显示返回入口；较早的会话仍可从会话列表打开。
...
助手工作期间仍可在输入框中准备下一条消息，草稿会保留；需要先停止当前任务，待停止完成后才能发送这条新消息。

没有发送过消息的新会话不会重复保存。未手动命名的会话会根据话题自动命名；手动修改名称后，助手不会再覆盖它。

关闭助手后再次打开会回到刚才的会话；刷新整个浏览器页面后则从一个新的空会话开始。若上一会话最近 10 分钟内仍有交互，消息区顶部会在 8 秒内提供“返回上一个会话”，需要继续刷新前的工作时可以直接恢复。会话目录在切换会话后保持展开，便于连续查看或批量管理。

在交互表单中选择项目空间、应用、集群等平台资源时，选择器仅显示易读的资源名称；提交后的会话消息会同时包含资源名称和 ID，便于确认目标并继续操作。

工作流需要项目空间、运行集群、镜像站、代码账号或其他已有资源时，助手会先检查当前可用候选。只有一个符合要求的候选时会直接采用并继续；存在多个有效候选、没有可用候选，或你明确要求更换或新建时，才会显示选择或创建交互。
...
诊断应用故障时，助手会围绕同一故障时间窗口核对应用拓扑、最近变更、Release、工作负载、事件、日志、指标和依赖关系，区分症状、直接原因、根因与待验证假设。要求修复时，助手会优先执行最小且可回滚的平台操作，并沿原取证路径重新验收；一次成功请求或一次重启不会被直接当作修复完成。
...
部署多服务项目时，助手会先生成服务拓扑，再决定依赖是复用现有实例、由当前解决方案共享、由单个服务独占，还是绑定外部托管服务。关系型数据库可以在兼容且隔离充分时复用服务器实例，并创建独立数据库、账号和密钥；Redis、Valkey 等缓存默认只在同一解决方案的 API、Worker、Agent 之间共享，不会默认跨无关业务复用。消息队列、对象存储、搜索、向量库、可观测和网关会分别按自身的租户隔离、版本、容量、生命周期和删除边界判断。执行前会展示哪些资源将新建、复用或绑定，删除时不会误删共享实例。
...
建议以单行胶囊按钮悬浮在消息区底部，不使用独立分隔栏；消息列表会保留足够的底部滚动空间，确保被候选项短暂遮挡的最后一条消息仍可滚动到完整可见。过长内容会省略；主建议、普通建议和高风险操作使用不同的语义样式。助手可以在整组建议都有明确视觉含义时统一配上 Emoji、平台图标或可信图片；无法保持整组一致时会只显示文字，避免为了装饰影响识别和排版。新建议从右侧平滑进入，使用减少动态效果的系统偏好时会自动关闭弹簧位移。

构建、发布、模板安装和钩子等持续任务会显示实时进度卡片。卡片绑定平台实际任务，并在状态变化时自动更新；刷新页面或短暂断线后会重新读取权威状态。无法取得真实任务标识时，助手不会用静态百分比模拟运行进度。任务达到终态后，助手仍会继续完成必要验收并给出结果回执。
...
如果模型生成的卡片结构未通过校验，助手会在同一个准备位置显示修正进度，并根据具体字段路径自动修正，最多尝试 3 次；修正成功后原位替换为最终卡片，不会重复插入卡片或要求刷新页面。达到上限后会保留失败状态和最后一次校验原因，助手也会在会话中说明卡在哪一步。
...
内部维护工具默认不会出现在普通会话中。平台管理员需要排查助手编排问题时，可以在助手顶栏开启调试模式；内部工具会按真实时间线位置显示并标注“内部工具”。该开关只影响当前管理员浏览器中的呈现，不会改变工具权限、执行结果或其他用户的界面。
