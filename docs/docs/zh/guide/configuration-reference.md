...
| 基本 | `REDIS_USERNAME` | 空 | Redis ACL 用户名；只使用 `requirepass` 时留空。 |
| 基本 | `REDIS_PASSWORD` | 空 | Redis 密码；完整 Compose 和内置 Helm Redis 默认要求配置。 |
| 基本 | `REDIS_DB` | `0` | Redis 逻辑数据库；API、Worker 和任务工具必须保持一致。 |
...
| 基本 | `REDIS_USERNAME` | 空 | Redis ACL 用户名；必须和 API 一致。 |
| 基本 | `REDIS_PASSWORD` | 空 | Redis 密码；必须和 API 一致。 |
| 基本 | `REDIS_DB` | `0` | Redis 逻辑数据库；必须和 API 一致。 |
