...
The approval card offers **Approve** and **Approve (don’t ask again in this conversation)**. The first approves only the call shown in the parameter summary. The second performs one shared MFA challenge, then allows tools requiring approval or step-up to be approved automatically in the current AI conversation and current browser session for up to 60 minutes. The assistant header shows the authorization state and latest possible expiry, and you can revoke it at any time. Signing out, changing browser sessions, changing the tool catalog or permissions, resetting MFA, or invalidating the security assertion revokes it. Every tool execution still rechecks the account, project authorization, tool policy, argument schema, and audit requirements. This authorization never applies to operations outside the AI conversation.

All MFA challenges use the same platform dialog and support both TOTP and one-time recovery codes. The platform reuses an active assertion for the same purpose within its security window. Users never copy a step-up credential into the assistant, and individual tools do not store verification codes.
...
When the platform has many tools, the assistant only uses capabilities that have been audited and explicitly admitted. The system evaluates relevant tools for the current goal. If the current set is insufficient, the assistant first reads a compact directory containing only operation IDs, categories, risks, and one-line purposes, then loads complete parameters by exact operation ID. It falls back to semantic search only when the compact directory does not identify a clear candidate, and continues the same task within the current turn. Directory browsing and search do not bypass your permissions or mean that a business operation has run, and you do not need to know tool names. Administrators can inspect these internal calls and results in Agent observability tool details.
...
When diagnosis requires several container commands that share a working directory or environment, the assistant can open a short-lived runtime command session. Every command revalidates permission, project policy, arguments, and audit policy. Without conversation auto-approval it still requests approval and MFA per command; with it enabled, the current conversation authorization can satisfy those two checks. Idle sessions and sessions that reach their maximum lifetime close automatically, and the assistant releases them when diagnosis ends. Do not use command sessions for long-running jobs or secret storage.
