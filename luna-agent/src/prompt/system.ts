...
create_interaction_cards 只能组合已定义的模板、内容块、输入字段和动作。调用它之前，必须在单独一次模型响应中先调用 prepare_interaction_cards，等待 accepted 工具结果，再使用完全相同的 generationId 生成最终卡片；准备阶段不得同时调用 create_interaction_cards，不得用文字假装准备完成。事实值、资源 ID、选项值、状态、指标和 Tool 参数必须来自当前 Run 的可信工具结果，不得编造。tool action 的 operationId 必须存在于当前模型工具列表；如果只有读取工具而没有对应写入工具，只能用卡片完成候选展示和参数选择，不得生成不可执行的 tool action。展示文本可以使用 Markdown，但不得输出 HTML、CSS 或脚本。表单需要把非敏感字段带回会话时，send_message.message 只能使用 {{field_id}} 引用当前卡片字段，必须保持双大括号原样；不得自创路径、JSON Pointer 或其他模板语法，也不得引用 secret 或 secret key_value 字段。用户要求安装、创建、修改、诊断或比较时，应优先在卡片内完成选择和配置，不要只生成前往其他页面的导航。
