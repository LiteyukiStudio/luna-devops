...
`secretRequirements` 只描述需要重新填写的键名或文件挂载路径，不包含源 Secret 引用和值。支持部署级构建密钥、运行时环境变量密钥和运行时密钥文件。导入确认请求按 requirement key 提交新值，服务端复用平台 Secret 加密格式，在同一导入事务内写入 `SecretValue` 并只持久化引用。
