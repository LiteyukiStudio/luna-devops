...
当前单个 Run 最多执行 48 次模型步骤、64 次平台工具调用。该值用于覆盖多轮读取、写入、回读验证以及
少量结构化参数修复，同时在模型重复调用或无法收敛时限制时长与成本；它只是安全
上限，不是正常任务的目标轮数或完成条件。正常流程应在用户目标达到验收标准、进入
等待状态或出现明确阻塞时尽早结束。达到上限时 Run 失败并保留未完成阶段，不能把
最后一张卡片或最后一次工具成功表述为业务完成。
