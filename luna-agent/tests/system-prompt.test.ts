...
describe("versioned system prompt", () => {
  ...
  it("loads the full tool catalog and does not require search before claiming a capability is missing", () => {
    const prompt = systemPromptFor("system-v4")

    expect(prompt).toContain("已全量加载全部已注册平台工具")
    expect(prompt).toContain("可直接选择使用，不需要先检索或猜测 operationId")
  })
  ...
  it("expands a plain GitHub deployment request into a verified multi-service delivery workflow", () => {
    ...
    expect(prompt).toContain("优先以 Kubernetes/Helm 资料确定工作负载类型")
    expect(prompt).toContain("这是一项有条件的证据优先级，不是机械规则")
    expect(prompt).toContain("Compose 中的开发专用 bind mount")
