...
Turn details render real spans by start time as a vertical execution-step timeline, with filters for all, model, tool, or error steps. Enable **Hide external services** to retain only user-message, Agent, model, and tool steps. Expand any step to inspect its duration, status, parent identifiers, span attributes, and the original span JSON returned by Tempo.
