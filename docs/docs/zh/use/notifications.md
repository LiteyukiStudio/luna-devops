...
通知会把需要关注的平台事件发送到协作工具或邮箱。默认先发送失败事件，避免每次成功构建、发布和入口同步都打扰管理员。

## 工作方式

一条通知从事件发生到真正送达，会经过四步：
...
构建、发布等业务模块只负责记录发生了什么，不需要了解飞书、企业微信、SMTP 或自定义 Webhook 的协议细节。
...
| `.Event.Message` | 失败摘要。 |
| `.Event.Project.Name` / `.Event.Project.Identifier` | 项目空间名称和不可变标识。 |
| `.Event.Application.Name` / `.Event.Application.Identifier` | 应用名称和不可变标识。 |
| `.Event.DeploymentTarget.Name` / `.Event.DeploymentTarget.Identifier` | 部署配置名称和阶段标识。 |
...
升级到不可变标识版本时，平台会自动把已保存通知模板和 Webhook 渠道配置中的 `.Event.Project.Slug`、`.Event.Application.Slug`、`.Event.DeploymentTarget.Slug` 改为对应的 `.Identifier` 字段，历史事件快照也会同步转换。
...
Docker Compose 部署更新后需要重新创建 Worker 容器，让 Worker 读取 `PUBLIC_BASE_URL`：
...
同一业务事件即使同时匹配全局规则和项目规则，每个通知渠道也只会收到一次投递。需要同时发往多个目的地时，应创建多个渠道。
