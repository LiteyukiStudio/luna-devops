...
生产模式必须同时提供 PostgreSQL 与 Luna API 配置。就绪探针会实时检查数据库 Schema 和 Provider
配置；任一依赖不可用时返回 `503`，Agent 不会继续接收新 Run。活动 Run 的增量事件只保留在当前
Agent 进程内，因此生产部署固定为单副本；进程重启会把未完成 Run 收敛为 `interrupted`，已经提交的
时间线与终态事实仍可从 PostgreSQL 回读。
...
| 配置项名称 | 默认值 | 说明 |
| --- | --- | --- |
| `AI_DATABASE_MAX_CONNECTIONS` | `10` | 限制 Agent 进程的 PostgreSQL 连接池；填写 `1`–`100` 的整数，并为 API 与 Worker 预留数据库连接预算。 |
...
Agent 使用固定的上下文边界：用户单轮文本最多为 128 KiB，历史超过 32 轮时压缩较旧内容并保留最近 16 轮。工具结果、摘要请求和续跑消息同样使用平台内置上限。这些限制用于保护请求传输和进程内存，不代表 Token 数，也不需要部署管理员调参。
