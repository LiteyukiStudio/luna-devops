...
func (h *Handler) ListInboxMessages(ctx *gin.Context) {
...
func (h *Handler) GetInboxUnreadCount(ctx *gin.Context) {
	user, ok := h.currentUser(ctx)
	if !ok {
		return
	}
...
func (h *Handler) GetInboxMessage(ctx *gin.Context) {
	user, ok := h.currentUser(ctx)
	if !ok {
		return
	}
...
func (h *Handler) MarkInboxMessageRead(ctx *gin.Context) {
	user, ok := h.currentUser(ctx)
	if !ok {
		return
	}
...
func (h *Handler) MarkAllInboxMessagesRead(ctx *gin.Context) {
	user, ok := h.currentUser(ctx)
	if !ok {
		return
	}
...
func (h *Handler) ArchiveInboxMessage(ctx *gin.Context) {
	user, ok := h.currentUser(ctx)
	if !ok {
		return
	}
...
func (h *Handler) DecideInboxActionRequest(ctx *gin.Context) {
	user, ok := h.currentUser(ctx)
	if !ok {
		return
	}
	...
	if err := h.decideInboxAction(ctx.Request.Context(), user, requestID, input.Decision, input.ExpectedVersion); err != nil {
		if errors.Is(err, ErrInboxDecisionUnavailable) {
			writeErrorCode(ctx, http.StatusServiceUnavailable, "inbox.decision_unavailable", "inbox decision handler is unavailable")
			return
		}
		WriteInboxError(ctx, err)
...
func (h *Handler) inboxMessageResponse(ctx context.Context, userID string, message model.InboxMessage) (InboxMessageResponse, error) {
...
func (h *Handler) inboxMessageResponses(ctx context.Context, userID string, messages []model.InboxMessage) ([]InboxMessageResponse, error) {
