    ...
    <Dialog open onOpenChange={open => !open && onClose()}>
      <DialogContent className="max-w-3xl">
        ...
        <FormGrid>
          ...
          <Field className="md:col-span-2" label={t('notificationsPage.configJson')}><Textarea className="min-h-40 font-mono" value={openState.configText} onChange={event => onUpdate({ ...openState, configText: event.target.value })} /></Field>
          <Field className="md:col-span-2" label={t('notificationsPage.secretLines')}><Textarea className="min-h-24 font-mono" placeholder={secretPlaceholder} value={openState.secretText} onChange={event => onUpdate({ ...openState, secretText: event.target.value })} /></Field>
        </FormGrid>
        <DialogFooter>
    ...
    <Dialog open onOpenChange={open => !open && onClose()}>
      <DialogContent>
        ...
        <FormGrid>
          ...
          <Field label={t('notificationsPage.name')}><Input value={openState.name} onChange={event => onUpdate({ ...openState, name: event.target.value })} /></Field>
          <Field className="md:col-span-2" label={t('notificationsPage.secretLines')}><Textarea className="font-mono" placeholder={(preset?.secretFields ?? []).map(field => `${field}=`).join('\n')} value={openState.secretText} onChange={event => onUpdate({ ...openState, secretText: event.target.value })} /></Field>
        </FormGrid>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>{t('common.cancel')}</Button>
    ...
    <Dialog open onOpenChange={open => !open && onClose()}>
      <DialogContent className="max-w-3xl">
        ...
        <FormGrid>
          ...
          <Field className="md:col-span-2" label={t('notificationsPage.subjectTemplate')}><Input value={openState.subjectTemplate} onChange={event => onUpdate({ ...openState, subjectTemplate: event.target.value })} /></Field>
          <Field className="md:col-span-2" label={t('notificationsPage.bodyTemplate')}><Textarea className="min-h-28 font-mono" value={openState.bodyTemplate} onChange={event => onUpdate({ ...openState, bodyTemplate: event.target.value })} /></Field>
          <Field className="md:col-span-2" label={t('notificationsPage.jsonBodyTemplate')}><Textarea className="min-h-40 font-mono" value={openState.jsonBodyTemplate} onChange={event => onUpdate({ ...openState, jsonBodyTemplate: event.target.value })} /></Field>
        </FormGrid>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>{t('common.cancel')}</Button>
          <Button disabled={saving} onClick={() => onSave(openState)}>{t('common.save')}</Button>
    ...
    <Dialog open onOpenChange={open => !open && onClose()}>
      <DialogContent className="max-w-3xl">
        ...
        <FormGrid>
          ...
          <Field className="md:col-span-2" label={t('notificationsPage.eventTypes')}><Input value={openState.eventTypes.join(', ')} onChange={event => onUpdate({ ...openState, eventTypes: splitList(event.target.value) })} /></Field>
          <Field className="md:col-span-2" label={t('notificationsPage.ruleScope')}>
            <Select
          ...
          {openState.scope === 'projects'
            ? (
                <Field className="md:col-span-2" label={t('notificationsPage.ruleProjects')}>
          ...
          <Field className="md:col-span-2" label={t('notificationsPage.filterJson')}>
          ...
          <Field className="md:col-span-2" label={t('notificationsPage.channels')}>
            <div className="grid gap-2 rounded-lg border border-border p-3">
              {channels.map(channel => (
                <label key={channel.id} className="flex items-center gap-2 text-sm">
                  <Checkbox checked={openState.channelIds.includes(channel.id)} onCheckedChange={checked => toggleChannel(channel.id, checked === true)} />
