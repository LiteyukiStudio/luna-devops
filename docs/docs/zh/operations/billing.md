...
页面顶部的余额是用户账户级概览；周期花费、今日花费和待结算金额会跟随当前项目空间范围统计。明细工具栏支持选择账单周期和项目空间范围：周期预设包含本周、近 7 天、本月、近 30 天、本年和去年，也可以用日期选择器自定义范围。费用分析、账单流水、用量记录和周期分类消耗都会按当前周期过滤。未选择项目空间时，普通用户默认查看自己名下的账单流水；平台管理员默认查看全部账单。
