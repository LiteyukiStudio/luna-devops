...
- `system-v3` 同时加载 `luna-devops-interaction` Skill，统一“决策 → send_message、
  执行 → request_tool、读取 → navigate/Markdown 链接”的动作选择。主回复与独立
  意图预测阶段必须使用相同规则；Provider 两次不兼容时只生成保持任务上下文的
  消息型兜底，不根据当前路由自动插入导航。
...
- 内建 `create_options` 是纯 UI 工具，不访问业务数据。输入包含标题、2-5 个选项及
  ...
  Provider 仍不兼容时才使用保持当前任务的消息型安全兜底。`request_tool` 只能转成新的用户
