...
`create_options` is a UI-only tool and does not read or mutate business data. Every normally completed response includes one user-intent prediction: the Agent derives 2-5 next-step options from the current question, the six most recent turns, page context, and trusted tool results, validates and durably stores them in the Timeline, and sends them to the browser over SSE. The UI renders them as always-visible Suggested next steps buttons instead of hiding them inside a collapsed generic Tool Call. If the Provider omits the tool, the Agent performs one required structured prediction pass; only a still-incompatible Provider falls back to safe message actions that preserve the current task context instead of switching pages. Each option has a stable ID, display label, optional description and tone, an independent repeat policy, plus exactly one action:
...
The Agent classifies options as decision, action, or read intent. When the answer asks for a missing project, application, cluster, environment, or another parameter, options must answer that question through **Send message**. Once the arguments are complete and the user wants a change, the Agent may suggest **Request operation**. **Navigate** is reserved for reading, browsing, or explicitly opening a resource. For example, project buttons under “Where should this be deployed?” send the selected trusted project ID back to the conversation instead of opening the project page. The navigation Skill owns safe routes, while the interaction Skill owns this workflow decision.
...
Platform Tool Call failures likewise expose only a stable localized reason, never SQL,
database-driver errors, or server stack traces. Expanding a failed Tool Call shows its
request ID. If the same action keeps failing, provide that ID to a platform administrator
so the corresponding server-side request can be located safely.
...
The available tool set is no longer filtered by the current page. Page context only helps the model understand the task. Project-scoped tools must explicitly submit a target `projectId`, while platform-scoped tools reject project arguments. Luna API revalidates the signed-in user, active session, and real RBAC Action on that target for every execution. A user can therefore continue work on an authorized project from a platform page such as the marketplace, but neither page content nor a model-supplied ID can expand the user's permissions.

For every new Turn, the Web client sends a versioned page-context envelope containing the registered route name and template, page kind, project and application IDs, active tab, allowlisted view state, selected resource IDs, available tabs, related internal routes, locale, time zone, and client timestamp. Tokens, callback targets, and other non-allowlisted query parameters are excluded. Luna API overwrites the locale and adds the server request time plus whether project page context is present; that marker is not an authorization decision. This metadata helps the model understand the current workspace but never replaces authorization on each tool call.
