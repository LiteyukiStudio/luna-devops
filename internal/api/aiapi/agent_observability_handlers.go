...
func (h *Handler) TestAgentObservabilitySource(ctx *gin.Context) {
...
func (h *Handler) GetAgentObservabilityOverview(ctx *gin.Context) {
	ctx.Header("Cache-Control", "no-store")
	values := h.allConfigValues()
...
func (h *Handler) GetAgentObservabilityTrace(ctx *gin.Context) {
	ctx.Header("Cache-Control", "no-store")
	user, ok := h.currentUser(ctx)
	if !ok {
		return
...
func (h *Handler) ListAgentObservabilityConversations(ctx *gin.Context) {
...
func (h *Handler) ListAgentObservabilityTurns(ctx *gin.Context) {
	user, ok := h.requireAgentObservabilityAdmin(ctx)
	if !ok {
		return
	}
...
func (h *Handler) ListAgentObservabilityTools(ctx *gin.Context) {
...
func (h *Handler) ListAgentObservabilityToolCalls(ctx *gin.Context) {
	user, ok := h.requireAgentObservabilityAdmin(ctx)
	if !ok {
		return
	}
...
func (h *Handler) GetAgentObservabilityConversation(ctx *gin.Context) {
	user, ok := h.requireAgentObservabilityAdmin(ctx)
	if !ok {
		return
	}
...
func (h *Handler) requireAgentObservabilityAdmin(ctx *gin.Context) (model.User, bool) {
	ctx.Header("Cache-Control", "no-store")
	user, ok := h.currentUser(ctx)
	if !ok {
...
func (h *Handler) resolveAppConfigSecret(ctx *gin.Context, key string) string {
	if key == "" {
		return ""
	}
	...
	return h.resolveSecret(ctx.Request.Context(), row.Value)
}
