...
运行集群也维护访问入口的默认域名后缀、访问链接协议和 Gateway API 默认值。访问入口按部署配置所在集群生成默认域名、补全短域名前缀，并返回控制台访问链接；因此多个集群可以分别接入不同 GatewayClass、共享 Gateway 或不同根域名。

网关端口分为两层：

- Gateway listener 端口是集群内部 Gateway/Controller 承接流量的端口，默认 `web:8080` 和 `websecure:8443`。访问入口会按集群访问协议自动绑定对应 listener，普通项目用户不需要选择端口或 listener。
- 访问端口是最终用户从外部访问应用时看到的端口，每个集群按当前访问协议配置一个。HTTP `80` 和 HTTPS `443` 不会显示在访问地址里；如果配置为非标端口，访问地址会显示 `:端口`。

`websecure` listener 的协议由外部 TLS 模式决定：选择“Gateway 终止 TLS”时生成 `protocol: HTTPS`，用于 Traefik `websecure` entryPoint 已启用 TLS 的场景；选择“上游代理已终止 TLS”时生成 `protocol: HTTP`，用于 CDN/Nginx 已解密后转发明文 HTTP 到 Traefik 的场景。

如果外层 Nginx/CDN/负载均衡器已经占用宿主机 `80/443`，可以让它把流量转发到集群 Gateway 的 `8080/8443` 等内部端口，平台生成的访问地址仍按运行集群的访问协议和访问端口展示。
