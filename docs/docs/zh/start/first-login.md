...
登录后可从内容区顶栏右侧的头像打开账号菜单。菜单提供账号设置、文档、GitHub 仓库和退出入口；退出登录会先要求二次确认，避免误触结束当前会话。

页面存在多个分区时，切换 Tab 会显示在同一顶栏的第二行，并在较窄屏幕上支持横向滚动。页面正文只保留当前分区的内容和必要操作，因此进入设置、资源管理或项目工作台后可以获得更完整的纵向工作空间。
...
- 填应用名称。
- 填短一点的英文标识。标识创建后不能修改；删除应用时，要等平台完成资源清理后才能复用。
- 先不处理高级运行参数。
...
继续进入 [连接集群和镜像站](/start/connect-resources)。
