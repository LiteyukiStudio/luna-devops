...
func (h *Handlers) DeleteRuntimeCluster(ctx *gin.Context) {
	...
	if !deleteStatusCanStart(cluster.DeleteStatus) {
	...
	if err := h.dbFor(ctx).Transaction(func(tx *gorm.DB) error {
		if err := markResourceDeleting(tx, &model.RuntimeCluster{}, cluster.ID); err != nil {
			return err
		}
	...
	if !h.enqueueResourceCleanup(ctx.Request.Context(), "runtime_cluster", cluster.ID, "", user.ID) {
		_ = markResourceDeleteFailed(h.dbFor(ctx), &model.RuntimeCluster{}, cluster.ID, "资源清理任务投递失败，请稍后重试")
