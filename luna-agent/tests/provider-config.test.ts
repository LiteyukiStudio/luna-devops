  ...
  it("uses only the callback service identity and parses no-store configuration", async () => {
    const fetchMock = vi.fn(async (_url: URL, init: RequestInit) => {
      ...
      return new Response(JSON.stringify({
        ...
        runtime: { providerTimeoutMs: 45_000, runTimeoutMs: 420_000, agentConcurrentRuns: 3, userConcurrentRuns: 10, contextInputTokenBudget: 256 * 1024 },
    ...
    expect(config.runtime).toEqual({ providerTimeoutMs: 45_000, runTimeoutMs: 420_000, agentConcurrentRuns: 3, userConcurrentRuns: 10, contextInputTokenBudget: 256 * 1024 })
