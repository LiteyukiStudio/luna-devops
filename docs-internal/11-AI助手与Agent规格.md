...
本文合并 AI 助手方向的全部内部方案与运行时参考，包含内嵌 AI 助手规格与 Agent 运行时/Skill 审计。交互卡片协议契约是现行规范，独立维护在 `docs-internal/12-AI声明式交互卡片Schema.md`。本文其余内容为方案与参考记录。
...
| 编排框架 | LangGraph.js；Luna PostgreSQL Run/Timeline 为权威状态，`ContextCompiler` 生成派生上下文 |
...
- StateGraph 适合表达可版本化的模型节点、工具循环和有界状态转换。
- 批准、补充输入和 MFA 等 Human-in-the-loop 状态由 Luna 持久化 Run/Tool Call 状态机承载。
- 支持流式输出和工具执行状态。
- 图节点和状态边界明确，便于测试、审计和控制循环次数。
- TypeScript 技术栈与 Web、共享 API Client 和未来独立 Agent 服务一致。
- 框架只承担图编排，不强制接管 Luna 已有的认证、权威时间线、数据库、HTTP、可观测
  和部署体系，接入边界清晰。

约束：

- `ai.conversations` / `ai.turns` / `ai.runs` / `ai.items` 是会话、执行和恢复的唯一权威状态源。当前不接入 LangGraph Checkpointer，避免另一套节点状态与 Luna Run/Timeline 双写漂移。
- 模型上下文由 Repository 与 Graph/Provider 之间的 `ContextCompiler` 编译；会话摘要是可重建的派生数据，不是 Run 检查点。
...
- 工具副作用必须由持久化 Tool Call、参数哈希和幂等键约束；Run 恢复时不得重复执行已完成的副作用。
- 摘要按 `coveredThroughTurnIndex` 增量生成并记录 `compressionVersion`；完整消息只按数据保留策略清理。
...
放回 `queued`，由任意健康副本重新取得租约，并从持久化 Run、Tool Call 和 Timeline 水位继续。
...
- 副本崩溃后，其他副本可在租约过期后按持久化 Run、Tool Call 和 Timeline 水位接管；不从第二套 Graph Checkpoint 恢复。
...
1. 立即停止领取新 Run。
2. 标记实例 draining。
3. 给当前模型或工具边界最多 20 秒完成或停止，不在副作用中间伪造可恢复状态。
4. 释放租约；其他副本仅依据已持久化的完整边界接管。
...
- 验证成功后把绑定的批准结果持久化，再从权威 Tool Call/Timeline 水位继续。
...
保存模型上下文所需的结构化派生摘要：`conversation_id`、
`covered_through_turn_index`、`compression_version`、`source_turn_count`、`content`
与时间戳。摘要只能单调向前更新，删除会话时级联删除；它不保存 Graph 节点或 Run
恢复状态，也不能代替完整 Timeline。
...
默认 `ai.access.mode=all_authenticated`，所有已登录且未禁用的用户可以使用助手；管理员
可以切换为 `admins`，此时普通用户能力接口返回 `available=false` 和
`ai.user_not_allowed`。访问范围只决定谁能启动 Agent，不改变工具按当前用户权限授权的规则。
...
`ContextCompiler` 位于 Repository 与 Graph/Provider 之间，在每次调用模型前按输入 Token 预算编译上下文：

- 固定系统与安全指令必须完整保留。
- 最近用户目标和当前页面上下文优先。
- 近期完整轮次按轮次边界取舍，不拆分用户/助手/工具调用与结果对。
- 采用高、低双水位：默认在输入预算达到 80% 时触发压缩，压缩目标回落到 50%，避免在模型极限附近反复失败；轮次数量只作为防止大量短消息无限堆积的第二触发条件。
- 正常压缩后保留最近 4 个完整轮次；未触发压缩时最多保留 8 个完整轮次。Token 压力超过预算时允许弹性缩减近期原文到最后 1 轮，但不能拆散当前 Run 的工具调用与结果。
- 较旧历史按 `coveredThroughTurnIndex` 增量合并到结构化会话摘要；摘要保存目标、约束、已确认资源、已完成操作、失败、待办和持久事实。
- 历史积压按有界批次追赶摘要水位；摘要覆盖区间必须连续，发现缺口时明确标记并在后续 Run 继续追赶，禁止跳过缺口后宣称已经覆盖。
- 摘要生成失败时安全回退到有界近期权威历史，不改写、删除或伪造完整时间线。
- 压缩模型的输入和输出各自具有独立预算；摘要失败、格式错误或 Provider 暂时不可用不能破坏会话，也不能让主模型收到超过输入预算的请求。
- 工具原始输出只保留本轮需要的字段；列表保留总数和有限样本。
- 已完成旧轮次的工具参数和结果先统一脱敏，再投影为有界摘要；当前 Run 的工具调用与结果按调用 ID 成对保留，并为结构化摘要预留固定预算。
- 日志默认取故障附近窗口，不能为了“更完整”自动读取全部历史。
- 工具输出超过 256 KiB 时先在服务端确定性裁剪，再交给模型摘要。

摘要本身视为不可信派生数据，保留已覆盖的 Turn 水位和压缩版本。涉及批准、参数、权限和资源
目标时必须回读结构化事实，不能只相信历史摘要。

完整 Timeline 始终是会话权威事实，摘要只是可重建的模型输入投影。不得同时启用另一套
LangGraph checkpointer 作为会话记忆，以免与 Run、Item 和摘要水位形成双重状态源。
...
- 使用 PostgreSQL 中的权威 Run、Tool Call 和 Timeline 状态恢复被中断的运行；不另行维护 LangGraph Checkpoint。
- 收到 SIGTERM 后停止接收新 Run，停止当前执行并依赖已持久化水位由其他副本重新领取。
...
副本按 PostgreSQL 租约调度。仅运行命令会话例外：它是 API owner 副本持有的短期 SPDY/Shell
连接，不是 AI 会话进程；它受空闲和绝对 TTL 约束，不写入 PostgreSQL 伪装可恢复。
...
| `ai.access.mode` | `all_authenticated` | `all_authenticated`、`admins`；只决定谁能启动 Agent，不提升工具权限 |
...
默认 `access.mode=all_authenticated`。平台需要限制入口时可切换为 `admins`；模型成本由
计费、并发和 Token 配额独立控制，不把成本策略混入用户是否可见助手的访问判断。
...
- 实现 LangGraph.js 主图、Luna 权威 Run/Timeline、Run 租约、心跳、取消和接管。
...
另一个副本能从权威 Run/Timeline 水位接管，且不会重复生成已持久化消息或工具结果。
...
升级，但替换 LangGraph.js、Luna 权威 Run/Timeline 持久化、Fastify 或独立 Agent 进程模型
属于架构变更，需要重新评审。
