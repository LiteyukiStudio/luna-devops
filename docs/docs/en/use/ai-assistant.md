...
The platform generates the Agent tool catalog automatically from backend OpenAPI operations; protocol adapters, authentication callbacks, and credential endpoints are excluded by one reasoned deny list. The assistant starts with `search_tools`: an empty query pages through the complete lightweight directory of operation IDs, names, summaries, tags, bilingual aliases, and high-risk flags, while a query performs targeted lookup. It then calls `get_tool_details` with exact operation IDs to load input and output schemas. Those schemas are not placed in the model's initial context. Search never bypasses authorization or means that a business operation has run.
