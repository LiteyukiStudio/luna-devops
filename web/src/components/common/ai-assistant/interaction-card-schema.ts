...
export const interactionFormFieldSchema = z.discriminatedUnion('type', [
  ...
  z.object({ ...fieldBase, type: z.literal('select'), defaultValue: z.string().max(240).optional(), placeholder: z.string().max(200).optional(), display: z.enum(['select', 'radio', 'segmented']).optional(), submissionFormat: z.enum(['value', 'label_value']).optional(), options: z.array(option).min(1).max(50) }),
  z.object({ ...fieldBase, type: z.literal('multi_select'), defaultValue: z.array(z.string()).max(50).optional(), placeholder: z.string().max(200).optional(), minItems: z.number().int().min(0).optional(), maxItems: z.number().int().positive().max(50).optional(), submissionFormat: z.enum(['value', 'label_value']).optional(), options: z.array(option).min(1).max(50) }),
