...
- [x] 全平台只使用 `credits` 作为内部货币，底层金额使用高精度 decimal 存储并支持小数，禁止使用 `float32/float64` 表示金额；展示单位、充值包和兑换关系后续由站点管理员在站点设置中配置。
...
- [ ] 前端提供项目空间账单视图：已展示余额、今日花费、本月花费、账本流水和用量记录；近期待扣用量、构建/运行/访问/存储分类消耗和低余额提示待补齐。
