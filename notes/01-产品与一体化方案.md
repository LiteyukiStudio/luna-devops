...
- Project 是协作和资源归属边界。
- Project identifier 是平台内永久唯一的不可变标识；Application identifier 在同一 Project 内永久唯一，软删除后也不允许复用。
...
- Project/Application identifier 长度为 2–22，stage 长度为 2–12，只允许小写英文字母、数字和短横线，并以字母或数字开头和结尾。该约束保证数据库 ID、Kubernetes label value 和 DNS label 资源名均不超过 63 字符。
