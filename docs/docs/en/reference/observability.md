...
Deterministic catalog browsing emits an `execute_tool browse_tools` span. A `list` result contains only the compact directory, while tools loaded by `details` should appear in the following `agent.tools.available` span. `luna_devops_agent_tool_directory_browse_total` counts directory calls by mode and outcome, and `luna_devops_agent_tool_directory_items` records the number of returned items. Dynamic semantic discovery still emits an `execute_tool search_tools` span with `gen_ai.operation.name=execute_tool` and `gen_ai.tool.name=search_tools`; the following `agent.tools.available` span should include the discovered tools. `luna_devops_agent_tool_searches` counts searches by outcome, while `luna_devops_agent_tool_search_matches` records the number of matches per search. Directory requests and search text are not placed in ordinary span attributes or metric labels; they are visible in controlled content attributes only when sensitive content capture is explicitly enabled.
