...
type DeploymentBuildEnvironmentInput struct {
...
func (h *Handlers) deploymentBuildEnvironmentFromInput(ctx *gin.Context, user model.User, projectID, targetID string, input DeploymentBuildEnvironmentInput, existing *model.BuildEnvironmentConfig) (*model.BuildEnvironmentConfig, bool) {
...
func (h *Handlers) buildEnvironmentSecretRefsFromInput(ctx *gin.Context, user model.User, scope, scopeRef string, input, existing map[string]string) (map[string]string, bool) {
	...
	for key, value := range input {
		...
		ref := h.storeSecret(ctx.Request.Context(), value, user.ID, "build_environment:"+scope+":"+scopeRef+":"+key)
