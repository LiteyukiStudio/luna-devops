...
The model catalog stores prices per one million tokens for ordinary prompt, completion, and cached prompt usage. Luna DevOps settles only schema-valid `prompt_tokens`, `completion_tokens`, and official details reported in Provider `usage`. `reasoning_tokens` is already included in completion usage and is never charged twice. Provider-reported cache-write prompt usage uses the cached-prompt price. AI cost is charged to the initiating user's personal wallet and is never assigned to a project space.
...
2. Check the display currency, Credit conversion, and all three model token prices.
