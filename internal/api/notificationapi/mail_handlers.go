...
func (h *Handler) GetPlatformMailSettings(ctx *gin.Context) {
...
func (h *Handler) UpdatePlatformMailSettings(ctx *gin.Context) {
	if !h.requirePlatformAdmin(ctx) {
		return
	}
...
func (h *Handler) TestPlatformMailSettings(ctx *gin.Context) {
	if !h.requirePlatformAdmin(ctx) {
		return
	}
	user, ok := h.currentUser(ctx)
	...
	recipient, err := h.normalizedRegistrationEmail(input.Recipient)
	if err != nil {
