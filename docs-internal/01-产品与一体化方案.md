...
- Project 是协作和资源归属边界。
- Project/Application 使用平台生成的随机、不可变数据库主键串联权限、审计、计费和历史；用户指定的 identifier 在资源有效期内保持不可修改并唯一。资源删除中或删除失败时仍占用 identifier，只有 Worker 完成外部资源清理并软删除业务记录后才允许复用。
...
用户管理页面用于 PlatformAdmin 管理统一平台账号。密码与 OIDC 外部身份是用户账号的可选登录方式，不再区分两类用户记录。
...
- 邮箱注册和 OIDC 注册分别由站点策略控制；邮箱默认关闭，OIDC 默认开启。
- 邮箱注册启用后通过 SMTP 发送一次性验证码，SMTP 密码保存在 Secret Store。
- OIDC 外部身份只作为统一用户的绑定关系，关闭 OIDC 注册不影响已有绑定用户登录。
- 禁用当前登录账号必须被后端拒绝，避免管理员把自己锁出系统。
- 用户列表采用紧凑两行摘要：第一行展示用户名和状态、角色、来源 Badge；第二行展示邮箱，语言偏好不在列表中展示。
- 新用户创建成功后，后端自动创建一个默认项目空间并将该用户设为 owner。默认项目空间名称按用户语言生成：中文为 `{用户名} 的项目空间`，英文为 `{Username}'s Project Space`；不可变标识从用户名或邮箱本地部分生成并自动避让冲突。
...
- 平台自身只发布容器镜像到 DockerHub `liteyukistudio` 命名空间，仅构建 `linux/amd64`，镜像为 `luna-devops`、`luna-worker`、`luna-agent`；分支构建发布 `nightly`，`v*` tag 发布版本 tag，稳定版本 tag 额外发布 `latest`，不额外构建或上传 GitHub Release 二进制产物。
...
| 用户 | Platform User | 所有用户统一落入平台用户表，密码和 OIDC 均为可选登录方式 |
...
- 邮箱注册默认关闭，OIDC 注册默认开启，两者都可由 PlatformAdmin 独立调整。
- 本地密码账号可由 PlatformAdmin 创建、邀请、导入，或在邮箱注册开启时由用户完成验证码注册。
...
- `allowEmailRegistration` 默认 `false`，启用前必须配置可用 SMTP。
- `allowOidcRegistration` 默认 `true`，关闭后只阻止新用户创建，已有绑定身份继续可用。
- `allowExternalIdentityPassword` 默认 `false`，决定无密码账号是否可以补设本地密码。
...
- 后端默认只读取 `.env`；显式设置 `ENV_FILE` 时使用指定文件替代 `.env`，进程环境变量始终优先。开发模式可以打印环境变量文件加载状态和文件路径，便于确认本地配置是否生效；生产模式不打印环境变量文件路径。
- 开发 Compose 与宿主机进程共用单一 `.env`。Compose 只从中插值，并在 worker 容器内显式设置数据库、Redis 等容器网络地址，避免宿主机的 `localhost` 地址在容器内失效。完整部署的 `docker-compose.yaml` 和 `docker-compose-build.yaml` 继续内联 API / worker 容器环境变量，生产密钥、域名和镜像 tag 通过宿主机环境变量覆盖。
...
- 镜像站凭据拆分两个维度：`usage` 表示 pull/push 用途，`scope` 使用 global/project/user 表示凭据可被谁使用。
- 凭据范围不能超过所属镜像站：用户镜像站只能创建个人凭据；项目镜像站可创建个人凭据或绑定到该镜像站项目范围子集的项目凭据；全局镜像站可创建个人、项目或全局凭据。
- 普通用户列表只展示可用镜像站摘要，不展示管理员维护的 endpoint、capabilities 等配置字段；资源维护者和平台管理员可以看到完整配置。
- 新建凭据默认设为个人使用；创建后可以在父级镜像站允许的范围内修改为项目空间或全局使用，项目范围支持多选。
...
- MVP 默认一个 Project 对应一个 Kubernetes Namespace。
- 新建 Project 的 Namespace 固定命名为 `luna-{projectIdentifier}`。
- 项目和应用 identifier 创建后不可修改；显示名称可以修改。新建 DeploymentTarget 的 Deployment、StatefulSet、Service 和 HPA 共用 `luna-{appIdentifier}-{stage}` 基名，ConfigMap、Secret 和 PVC 在该基名后追加语义后缀，Pod 内主容器名固定为 `app`。
- Project/Application identifier 长度为 2–22，stage 长度为 2–12，只允许小写英文字母、数字和短横线，并以字母或数字开头和结尾。该约束用于生成稳定、可读且不超过 63 字符的 Kubernetes DNS label 资源名；数据库主键由平台独立随机生成。
- Kubernetes 名称单独持久化。迁移前已经创建的项目和部署配置继续使用原 `ns-*` / `dplt-*` 名称，升级不会重命名或重建现有运行资源；平台识别和同步继续依赖稳定 ID labels。
...
```text
Deployment:  luna-{appIdentifier}-{stage}
StatefulSet: luna-{appIdentifier}-{stage}
Service:     luna-{appIdentifier}-{stage}
HPA:         luna-{appIdentifier}-{stage}
ConfigMap:   luna-{appIdentifier}-{stage}-config
Secret:      luna-{appIdentifier}-{stage}-secret
```
...
```yaml
labels:
  luna.devops/project-id: "{projectId}"
  luna.devops/application-id: "{applicationId}"
  luna.devops/environment-id: "{environmentId}"
...
- 应用目标镜像引用的默认生成策略为 `{镜像站用户名}/{projectIdentifier}-{appIdentifier}:latest`，其中镜像站用户名来自 `ArtifactRegistry.namespace`；namespace 为空时使用当前项目空间不可变标识兜底，生成 `{projectIdentifier}/{projectIdentifier}-{appIdentifier}:latest`。
...
```text
dev:     {projectIdentifier}-{appIdentifier}-dev.{rootDomain}
test:    {projectIdentifier}-{appIdentifier}-test.{rootDomain}
staging: {projectIdentifier}-{appIdentifier}-staging.{rootDomain}
prod:    {projectIdentifier}-{appIdentifier}-prod.{rootDomain}
```

如果出现冲突：

```text
{projectIdentifier}-{appIdentifier}-{stage}-2.{rootDomain}
{projectIdentifier}-{appIdentifier}-{stage}-3.{rootDomain}
```
...
- Release 创建和回滚请求会投递 `deploy:run` 任务；Worker 对新项目按持久化的 `luna-{projectIdentifier}` 幂等创建和使用 Project namespace，历史项目继续使用迁移保留的原 namespace。
- 部署 Worker 对新部署配置以持久化的 `luna-{appIdentifier}-{stage}` 作为应用资源基名，按 Release.imageRef 下发 ConfigMap、Secret、Deployment/StatefulSet 和 Service，并轮询 rollout 状态；历史部署配置继续使用迁移保留的原资源名。rollout 成功会把 Release 标记为 `succeeded`，失败或超时会标记为 `failed` 并记录原因。Workload selector 只使用项目、应用、环境、部署配置等稳定 ID 标签，不包含每次发布都会变化的 release-id，避免二次部署触发 Kubernetes immutable selector 错误。
...
- Registry 凭据只在当前构建所属项目空间内选择，优先级为该镜像站的可用默认凭据、触发人的 `user` push/push-pull 凭据、当前项目绑定的 `project` 凭据、`global` 凭据。
...
- 内容区内需要二级切换时，统一使用 `ContentTabs`：tab 在左，当前 tab 的工具按钮在右；桌面端水平对齐并使用紧凑 tab，移动端改为“当前 tab + 下拉选择”，工具按钮自然换行。
...
- 横向单选切换统一使用 `SegmentedControl` 视觉样式；主题模式等真正互斥模式可以使用胶囊活动态，页面导航与普通命令控件不得为了外观统一而伪装成分段模式。侧边栏主题切换空间较窄，只展示图标，文案保留为无障碍标签。
- 圆角按组件语义分层：普通按钮、输入框、选择器和侧边栏导航使用 Tailwind `rounded-md` / `rounded-lg`；卡片、弹窗和渐进区块使用 `rounded-lg`；状态 Badge、头像、模式分段和明确的圆形图标按钮才使用 `rounded-full`。业务页面不得用 `rounded-full` 覆盖基础控件默认样式。
- 前端尺寸必须优先使用 Tailwind spacing、size、radius、grid 和 typography token。除视口比例、CSS 变量及必须表达动态计算的布局外，不新增任意 `px` 值或页面级魔法尺寸。
- 运维页面以分隔线、留白和背景层级组织内容，避免卡片嵌套卡片。看板统计可以使用统一边界分组，但少量告警不得独占大面积空白区域。
...
- 凭据拆分两个维度：`usage` 表示用途，取值为 `pull`、`push`、`push-pull`；`scope` 表示可使用范围，取值为 `user`、`project`、`global`，项目级可绑定多个项目空间。
- 凭据作用域必须包含在父级镜像站作用域内：用户镜像站只允许 `user`；项目镜像站允许 `user`，或绑定到父级项目范围子集的 `project`；全局镜像站允许三种作用域。
- 新建凭据默认使用 `user`，避免创建时无意扩大密钥使用范围；后续可由具备对应维护权限的用户显式调整。
- 凭据密文永不回显。普通使用者只能引用和使用凭据，不能查看用户名以外的敏感字段、密码或 Token。
- `user` 凭据由创建者维护；`project` 凭据由绑定项目空间的 Owner/Admin 维护；`global` 凭据仅由平台管理员维护。
- 普通镜像站凭据列表只展示当前用户自己的 `user` 凭据、当前项目空间可用的 `project` 凭据和 `global` 凭据；平台管理员不在该业务入口混看其他用户的 `user` 凭据，后续如需审计管理必须单独建设管理视图。
- 部署和构建按当前项目空间精确选择凭据，优先级为可用默认凭据、当前用户的 `user` 凭据、当前项目绑定的 `project` 凭据、`global` 凭据，不能使用用户其他项目空间的凭据。
