...
func TestCrossProjectListVisibilityPostgres(t *testing.T) {
	...
	projects := []model.Project{
		{ID: "prj_visibility_related", Identifier: "visibility-related", Name: "Related", DeleteStatus: "active"},
		{ID: "prj_visibility_other", Identifier: "visibility-other", Name: "Other", DeleteStatus: "active"},
		{ID: "prj_visibility_developer", Identifier: "visibility-developer", Name: "Developer", DeleteStatus: "active"},
		{ID: "prj_visibility_viewer", Identifier: "visibility-viewer", Name: "Viewer", DeleteStatus: "active"},
	}
	...
	clusters := []model.RuntimeCluster{
		{ID: "rcl_visibility_global", Name: "Global", Scope: "global"},
		{ID: "rcl_visibility_user", Name: "Current user", Scope: "user", OwnerRef: admin.ID},
		{ID: "rcl_visibility_other_user", Name: "Other user", Scope: "user", OwnerRef: other.ID},
		{ID: "rcl_visibility_related", Name: "Related project", Scope: "project"},
		{ID: "rcl_visibility_other", Name: "Other project", Scope: "project"},
		{ID: "rcl_visibility_unbound", Name: "Unbound project scope", Scope: "project"},
	}
	...
	t.Run("cluster resources include every member role", func(t *testing.T) {
		...
		developerItems := handlers.domains.runtime.FilterClusterResourceSnapshots(visibilityContext(developer, ""), developer, items, projectservice.ListVisibilityRelated, "")
		assertSnapshotIDs(t, developerItems, []string{"kres_developer"})
		viewerItems := handlers.domains.runtime.FilterClusterResourceSnapshots(visibilityContext(viewer, ""), viewer, items, projectservice.ListVisibilityRelated, "")
		assertSnapshotIDs(t, viewerItems, []string{"kres_viewer"})
		adminRelated := handlers.domains.runtime.FilterClusterResourceSnapshots(visibilityContext(admin, ""), admin, items, projectservice.ListVisibilityRelated, "")
		assertSnapshotIDs(t, adminRelated, nil)
		adminAll := handlers.domains.runtime.FilterClusterResourceSnapshots(visibilityContext(admin, "all"), admin, items, projectservice.ListVisibilityAll, "")
		assertSnapshotIDs(t, adminAll, []string{"kres_developer", "kres_viewer", "kres_other", "kres_unscoped"})
		explicit := handlers.domains.runtime.FilterClusterResourceSnapshots(visibilityContext(admin, ""), admin, items, projectservice.ListVisibilityRelated, projects[1].ID)
...
func listContainerImageIDs(t *testing.T, handlers *Handlers, user model.User, visibility string) []string {
	...
	handlers.domains.registry.ListContainerImages(ctx)
