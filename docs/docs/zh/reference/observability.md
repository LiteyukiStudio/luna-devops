...
配置完成后进入“运营面板 → Agent 观测”，可以查看 Run 成功率、活跃 Run、首 Token P95、模型与依赖错误率、Token 吞吐、工具失败趋势、最近 Agent Run 和 Trace 关联失败日志。调用链工作区支持“按会话”和“按轮次”两种视角：平台管理员可以跨用户检索并分页查看会话，在会话详情中分页查看用户消息、Agent 回复、各轮状态和关联 Trace；也可以按轮次跨会话检索并分页定位单次执行。每轮用户消息通常产生一条 Trace，同一会话的多轮 Trace 通过 Conversation ID 关联。

会话详情会按 Agent 的实际时间线把每轮拆成多次 Loop，展示思考摘要、安全 Markdown 回复、工具参数与脱敏结果。打开任一轮的“查看调用链”，或从某次工具调用选择“查看工具调用链”，同一详情页会先回放关联的用户消息、Thinking、模型回复及工具输入输出，再通过默认完整展开的瀑布视图展示 Span 父子关系、并行阶段、耗时占比、错误节点，以及模型、工具、HTTP 和数据库调用；从工具记录可以直接定位对应 Span，Span 树支持逐节点或整体折叠与展开。会话消息从 Luna 的权威会话存储读取，不写入 Tempo、Loki 或 Prometheus；Tempo 属性侧栏仍只展示经过白名单筛选的模型、Token、工具标识和错误属性，不展示 Prompt 或工具敏感参数。跨用户会话与消息仅平台管理员可见，详情读取会写入审计日志。
