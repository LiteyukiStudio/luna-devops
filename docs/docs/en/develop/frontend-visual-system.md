...
| --- | --- | --- |
| Resource list | `full` | `PageChrome` + `DataList` |
...
`DataList` is already a complete list and should not be wrapped in another Card. The shared `TableFrame` uses one border and rounded boundary for the scrollable table body and its optional footer, with pagination rendered inside that footer instead of a separate container below the table. The toolbar, header, body, and footer own their internal spacing without adding duplicate whitespace around the table. Keep one solid primary action per page by default. Search, filters, sorting, and refresh actions belong in `DataList.toolbar` or `ContentTabs.tools`.
...
- `DataList` does not create a Card shell. The shared `TableFrame` owns the scrollable table viewport and optional footer through one native semantic border and one rounded clipping boundary. Pagination belongs to the footer and stays outside the horizontal scrolling region, so it remains visible while wide table content scrolls. Do not simulate the boundary with nested backgrounds and padding because sticky headers can expose antialiasing seams at rounded corners. Inside the table, keep only the necessary separators between data rows and one separator between the body and footer. When a list has no title, search, filters, or bulk-action toolbar, `DataList` automatically keeps one `spacing-group` above the table, so business pages must not add compensating spacing. Mobile table cells use compact inline spacing; single-page lists show only the item summary, while page-size and navigation controls appear only for multi-page results in a responsive two-level footer.
- Place list search, filtering, sorting, and refresh controls in the `DataList` header toolbar and align them from the left. When the page title already identifies the list context, do not repeat labels such as “User list” inside the list shell or render an empty leading slot; the page title, query controls, and table frame must share the same left baseline. Use `DataList.title` only when multiple independent lists on the same page need explicit differentiation. Page-level primary actions such as create belong on the right side of the `PageChrome` title row. Do not add a separate query toolbar above the list. Use `DataList.toolbar` for filters, sorting, and refresh controls beyond the built-in search field.
...
- On desktop, the login illustration and form share one low-saturation brand-gradient card instead of using disconnected solid backgrounds for each column or container padding that creates a frame-within-a-frame. The main card and both column backgrounds use the standard `rounded-container` token. The form side may add a light continuous scrim from transparent to the surface color for text and control clarity, but it must not become a second inset card. Light and dark values are centralized in the `auth-card-*` semantic tokens. Mobile hides the illustration and keeps one solid `surface` Card with smaller viewport gutters and container padding, avoiding gradient distraction and layered whitespace that squeezes the form.
...
- The authenticated app root uses the low-saturation `workspace-background`, while the desktop sidebar remains transparent and directly inherits that global canvas without its own fill, right divider, or menu-group rules. Group labels and vertical spacing distinguish navigation categories. Navigation hover uses the light `sidebar-nav-hover`; selection adds only one restrained step of theme tint and must not approach a solid primary-action surface. Single-color themes use a subdued solid selection, while multicolor themes may use a low-saturation primary-to-secondary gradient with text mixed back toward the regular foreground. Selection remains only slightly stronger than hover. In Standard mode, `separator-*` may receive a restrained tint from the theme secondary role; Minimal mode keeps separators neutral. The mobile drawer remains an opaque themed overlay.
...
- Use neutral solid surfaces for large dashboard regions and every `MetricItem` tile. Reserve semantic color for exception icons, critical values, and necessary status badges instead of tinting the whole metric tile. `Notice` supports `variant="neutral"` to retain semantic meaning with less visual noise.
