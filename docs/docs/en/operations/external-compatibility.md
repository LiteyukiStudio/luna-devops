...
| cert-manager | `cert-manager.io/v1` Certificate | cert-manager `>= 1.0`; choose a release supported by the current Kubernetes version | Current maintained cert-manager release | The first Gateway API phase mainly creates HTTP-protocol listeners. HTTPS certificateRefs automation remains a follow-up item. |
