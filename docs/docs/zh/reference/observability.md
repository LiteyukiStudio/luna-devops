...
会话详情会按 Agent 的实际时间线把每轮拆成多次 Loop，展示思考摘要、安全 Markdown 回复、工具参数与脱敏结果。打开任一轮的“查看调用链”，或从某次工具调用选择“查看工具调用链”，可以使用默认完整展开的内嵌瀑布视图检查 Span 父子关系、并行阶段、耗时占比、错误节点，以及模型、工具、HTTP 和数据库调用；Span 树支持逐节点或整体折叠与展开。会话消息从 Luna 的权威会话存储读取，不写入 Tempo、Loki 或 Prometheus；Tempo 属性侧栏仍只展示经过白名单筛选的模型、Token、工具和错误属性，不展示 Prompt 或工具敏感参数。跨用户会话与消息仅平台管理员可见，详情读取会写入审计日志。
