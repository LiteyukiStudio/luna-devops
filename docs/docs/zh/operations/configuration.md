...
敏感操作二次验证由站点配置 `security.stepUpMfa.enabled` 控制，默认关闭。启用前，至少一名可用的平台管理员必须先在“账号安全”绑定离线 TOTP 验证器；全局策略开启后，最后一名已绑定 MFA 的平台管理员不能解绑。`security.stepUpMfa.idleTimeoutMinutes` 控制没有继续执行敏感操作时的空闲有效期，默认 10 分钟；`security.stepUpMfa.absoluteTimeoutMinutes` 控制一次验证无论是否持续活动都不能超过的绝对有效期，默认 60 分钟。

用户开始绑定后会看到二维码、完整 `otpauth` URI 和手动密钥。只有输入有效的 6 位 TOTP 后才会正式启用；校验接受当前 30 秒窗口及前后各一个窗口。确认时平台生成 10 个一次性恢复码，明文只展示一次，后端只保存 bcrypt hash。每个恢复码只能成功使用一次，重新生成会立即作废全部旧码。TOTP secret 进入平台加密 Secret 存储，不落业务表明文，也不会提供给管理员查看。
...
MFA 绑定、确认和验证按用户与来源 IP 限流，验证码和恢复码不会写入日志。使用恢复码、重新生成恢复码、绑定、解绑、策略更新以及验证成功或失败都会写入审计日志。账号密码、角色或禁用状态变化会撤销现有会话、remember token 和 Step-up assertion；解绑 MFA 也会删除 TOTP secret、恢复码及当前断言。
...
平台管理员可以在工作负载页的 Pod 子行打开 Web Console，直接进入该 Pod 的交互式终端。前端会先通过普通 HTTP API 调用 Pod terminal authorize 预检；如果返回 `mfa_required`，统一 MFA Dialog 可以完成 `runtime_terminal` 验证并自动重试。预检通过只表示可以继续尝试连接，随后 WebSocket 端点仍会重新校验会话、平台管理员角色、MFA assertion、Pod 身份和平台资源归属，再决定是否升级连接。终端操作会写入审计日志。
