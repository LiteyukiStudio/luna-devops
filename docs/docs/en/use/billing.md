...
Traffic collection is optional and is not installed by default. The billing page does not treat the system component installation record as the probe's current runtime state. Luna reads the probe workload from the target Kubernetes cluster for each status request and shows **Unavailable** when the cluster cannot be reached. It shows **Not deployed** when the workload is missing, **Waiting for report** when the workload is ready but no valid positive traffic window has arrived, and traffic spend after the first valid window. Probe online state is not cached, while settled usage remains immutable billing history. Platform administrators can install the `Luna Gateway Traffic Probe` platform component from “App Marketplace” and choose the target runtime cluster.
