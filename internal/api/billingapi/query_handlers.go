...
func (h *Handler) ListBillingLedgerEntries(ctx *gin.Context) {
...
func (h *Handler) ListBillingUsageRecords(ctx *gin.Context) {
	user, ok := h.currentUser(ctx)
	if !ok {
		return
	}
...
func (h *Handler) ListBillingDeploymentSpend(ctx *gin.Context) {
	user, ok := h.currentUser(ctx)
	if !ok {
		return
	}
