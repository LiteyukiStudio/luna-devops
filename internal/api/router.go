...
func NewRouterWithStaticFSAndMetrics(db *gorm.DB, staticFS fs.FS, httpMetrics *observability.HTTPMetrics) *gin.Engine {
	...
	router.POST("/internal/v1/ai/delegations/exchange", handlers.ExchangeAIDelegation)
	router.POST("/internal/v1/ai/tools/:operationId/execute", handlers.ExecuteAIInternalTool)
	router.POST("/internal/v1/ai/tools/:operationId/verify", handlers.VerifyAIInternalTool)
	...
	v1 := router.Group("/api/v1")
	{
		...
		v1.PUT("/auth/registration/settings", handlers.platformAdminMiddleware(), handlers.stepUpMiddleware(stepUpPurposeSecuritySettingsUpdate), handlers.UpdateAuthRegistrationSettings)
		...
		v1.POST("/auth/providers", handlers.platformAdminMiddleware(), handlers.stepUpMiddleware(stepUpPurposeAuthProviderUpdate), handlers.CreateAuthProvider)
		v1.PUT("/auth/providers/:providerId", handlers.platformAdminMiddleware(), handlers.stepUpMiddleware(stepUpPurposeAuthProviderUpdate), handlers.UpdateAuthProvider)
		...
		v1.GET("/ai/conversations/:conversationId/authorization", handlers.ProxyAIRequest)
		v1.DELETE("/ai/conversations/:conversationId/authorization", handlers.ProxyAIRequest)
		...
		v1.POST("/users", handlers.platformAdminMiddleware(), handlers.stepUpMiddleware(stepUpPurposeUserAdminUpdate), handlers.CreateUser)
		v1.PUT("/users/:userId", handlers.platformAdminMiddleware(), handlers.stepUpMiddleware(stepUpPurposeUserAdminUpdate), handlers.UpdateUser)
		...
		v1.POST("/data-retention/cleanup", handlers.platformAdminMiddleware(), handlers.stepUpMiddleware(stepUpPurposeDataRetentionCleanup), handlers.CleanupDataRetention)
		...
		v1.POST("/projects/:projectId/releases/:releaseId/command-sessions", handlers.CreateReleaseRuntimeCommandSession)
		v1.POST("/projects/:projectId/releases/:releaseId/command-sessions/:sessionId/commands", handlers.ExecuteReleaseRuntimeCommandSession)
		v1.DELETE("/projects/:projectId/releases/:releaseId/command-sessions/:sessionId", handlers.CloseReleaseRuntimeCommandSession)
