...
配置完成后进入“运营面板 → Agent 观测”。页面顶部可选择 1 小时、6 小时、24 小时、7 日、30 日或 1 年周期，并汇总输入 Token、输出 Token、工具调用、对话轮数量、终态轮次成功率和执行耗时 P95；下方按“从一次用户输入到 Agent 输出结束”为一轮，提供跨用户搜索和服务端分页。点击任意轮次会从右侧打开详情，不再提供会话级观测视图。

轮次详情按开始时间把真实 Span 呈现为纵向步骤时间轴，并支持筛选全部、模型、工具或异常步骤。用户消息接收、Agent 启动、模型生成、工具调用、状态持久化和外部请求均由对应 Span 驱动；展开步骤可以查看耗时、状态、父子标识和经过白名单筛选的安全属性。用户消息与 Agent 回复从 Luna 的权威会话存储读取，不写入 Tempo、Loki 或 Prometheus；Prompt、Secret 和工具敏感参数不会出现在属性列表中。跨用户轮次与内容仅平台管理员可见，Trace 详情读取会写入审计日志。
