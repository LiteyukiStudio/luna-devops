...
A Run no longer has a cumulative token or credit limit, so a long task is not forced into a new task by a site-wide usage threshold. Before each model request, the platform still constrains output by the model context, per-call output capability, and the user's available personal-wallet balance. It then settles the normal input, output, cached input, and cached output usage reported by the Provider. A request is not sent when the context or wallet cannot cover it.

Each Run also has a high tool-call safety guard. It defaults to 256 platform-tool calls and can be configured from 32 to 2048. The assistant separately limits repeated calls with the same operation and normalized arguments inside that Run. When a limit is reached, it reports why it stopped instead of claiming success; asynchronous work still requires an authoritative platform-state read before a terminal result is reported.

The single ring at the bottom of the composer shows the context usage reported by the latest main assistant model call. Hover or focus the ring to see that call's context usage, model context limit, and percentage. Reopening a conversation restores the latest usage from the authoritative timeline.

**Advanced settings** now keeps only runtime parameters that require platform-wide management. The tool-result context limit, compression trigger/target ratios, recent-turn count, maximum recent-turn count, and historical tool-result limit are no longer stored in platform configuration. They are stateless Agent policies and require no configuration by default. Deployment administrators can override them with Agent environment variables only when an environment has a concrete need; see the [Environment Variable Reference](../reference/configuration.md#agent-settings). Model context, per-call output capability, and all four prices remain model-specific settings in the model catalog.
...
To discard platform runtime tuning, select **Restore defaults** beside **Save settings**. Provider details, API keys, the model catalog, access scope, observability, and proxy settings stay unchanged. Review the restored values, then select **Save settings** to apply them. Agent environment variables are not changed by this console action.
...
Runtime secrets for a deployment target can be submitted either by the assistant through the dedicated tool or through the security form. Both paths use the same argument validation and approval flow, and the platform does not block the call based on its input source. Generated credentials are created and stored by the platform backend, so neither the assistant nor the browser receives the generated plaintext. Clearing requires a separate explicit clear action—an empty input never clears an existing value. Callers explicitly choose ordinary environment variables as `public`; the platform no longer infers sensitivity from names, URLs, or values and does not block them. Choosing an ordinary variable means the value does not receive secret encryption or non-disclosure guarantees. Approval summaries and results report only which secret fields will be set, generated, or cleared and their status; they never return plaintext secrets.

After a card submits a platform operation, a new turn immediately shows the real waiting-for-approval, succeeded, or structured-failure state. Array arguments such as secret-item lists are assembled at their declared card paths. The platform still validates the complete tool schema and returns invalid field paths and allowed values instead of reducing every validation failure to a generic missing-input prompt.

When the assistant explicitly requests a choice in its current reply, the options appear inside that bubble at their real position among messages, reasoning, and tool calls. The platform does not make a separate model call after the reply to predict next steps. Before the first message in a new conversation, page presets remain above the composer for quick access to common tasks.

When a high-risk tool is waiting for approval, its action card appears directly below the tool row without requiring you to expand the details. Parameters, results, and invocation identifiers remain collapsed until you choose to inspect them.

The approval card offers **Reject**, **Approve once**, and **Always allow this tool**. Approve once applies only to the current ToolCall. Always allow is stored only for the current account and the same `operationId`; it never applies to another account or tool and can be revoked from assistant settings. Every execution still rechecks the sign-in session, conversation ownership, account state, project authorization, argument schema, and audit requirements.
...
The platform generates the Agent tool catalog automatically from backend OpenAPI operations; protocol adapters, authentication callbacks, and credential endpoints are excluded by one reasoned deny list. A bilingual `search_tools` query directly adds a small set of the most relevant candidates to the current task so the assistant can call them immediately; an empty query only pages through the directory. The assistant uses `get_tool_details` only when exact operation IDs need semantic disambiguation. Full input and output schemas are loaded only with actual model tool definitions, never in the initial context. A task reuses its selected tools and repeated lookups return the same cached result. After an administrator updates the catalog, new tasks use the new version while in-progress tasks keep their creation-time version. Search never bypasses authorization or means that a business operation has run.

Public-web access is provided by the catalog operations `webSearch` and `fetchWebPage`: the former searches public pages, while the latter reads a user-selected public text page. Every request passes through the platform's egress policy and proxy pool. Private or reserved addresses, credential-bearing URLs, policy-blocked targets, and unsupported content types are rejected. Returned page text is treated as untrusted external data; requests never carry browser cookies, sessions, tokens, or Git credentials, and a search result grants no additional platform permission.

When creating asynchronous resources such as releases, builds, or gateway routes, the assistant extracts the real resource ID from the platform result and invokes the relevant detail or status operation for an authoritative read. An accepted HTTP request does not mean that the task has succeeded. A service interruption does not transfer or resume a Run from the middle of its call stack: the Run becomes interrupted while all prior messages, tool calls, cards, and error events remain available, and you can start the task again from that evidence.
...
When diagnosis requires several container commands that share a working directory or environment, the assistant can open a short-lived runtime command session. Every command requires approval bound to its current arguments and revalidates the sign-in session, permission, project policy, arguments, and audit policy. Idle sessions and sessions that reach their maximum lifetime close automatically, and the assistant releases them when diagnosis ends. Do not use command sessions for long-running jobs or secret storage.
...
### The context or wallet balance is insufficient

For a context error, shorten the current input, remove unneeded history, or ask an administrator to verify the model capabilities. If the personal wallet is insufficient, recharge it or choose an affordable model. The platform does not stop a task because of cumulative Run tokens or credits; context and wallet failures still stop before a new model charge is incurred.
