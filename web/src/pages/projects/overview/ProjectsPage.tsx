...
function ProjectSummary({ project }: { project: Project }) {
  const { t } = useTranslation()
  const deleting = project.deleteStatus === 'deleting'
  const deleteFailedMessage = project.deleteMessage?.trim() ?? ''
  return (
    <div className="flex min-w-0 items-center gap-3">
      ...
      <div className="min-w-0 w-full">
        ...
        <ResourceDeletionStatus
          className="mt-1"
          message={project.deleteMessage}
          messageClassName="max-w-60"
          messagePreview={deleteFailedMessage ? compactProjectDeleteMessage(deleteFailedMessage, t) : undefined}
          status={project.deleteStatus}
        />
