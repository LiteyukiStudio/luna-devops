...
describe("ProviderConfigClient", () => {
  it("uses only the callback service identity and parses no-store configuration", async () => {
    const fetchMock = vi.fn(async (_url: URL, init: RequestInit) => {
      ...
      return new Response(JSON.stringify({
        ...
        runtime: { providerTimeoutMs: 45_000, runTimeoutMs: 420_000, agentConcurrentRuns: 3, contextInputTokenBudget: 256 * 1024 },
    ...
    expect(config.runtime).toEqual({ providerTimeoutMs: 45_000, runTimeoutMs: 420_000, agentConcurrentRuns: 3, contextInputTokenBudget: 256 * 1024 })
  ...
  it("rejects runtime settings outside the platform contract", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => new Response(JSON.stringify({
      ...
      runtime: { providerTimeoutMs: 0, runTimeoutMs: 1_000, agentConcurrentRuns: 100, contextInputTokenBudget: 32 * 1024 },
    }), { status: 200 })))
