...
func (h *Handlers) DeleteProjectRuntimeConfigSet(ctx *gin.Context) {
	...
	if !h.deleteStatusCanStart(set.DeleteStatus) {
		writeError(ctx, http.StatusConflict, "运行配置正在删除中，请等待资源清理完成")
		return
	}
	if err := h.markResourceDeleting(h.dbFor(ctx), &model.ProjectRuntimeConfigSet{}, set.ID); err != nil {
	...
	if !h.enqueueResourceCleanup(ctx.Request.Context(), "runtime_config", set.ID, set.ProjectID, user.ID) {
		_ = h.markResourceDeleteFailed(h.dbFor(ctx), &model.ProjectRuntimeConfigSet{}, set.ID, "资源清理任务投递失败，请稍后重试")
