export class KeyValueTextError extends Error {
  ...
  constructor(code: 'empty_key' | 'duplicate_key' | 'invalid_line', message: string) {
    super(message)
    this.code = code
...
export function parseKeyValueText(value: string): Record<string, string> {
  ...
  for (const rawLine of value.split('\n')) {
    ...
    if (separator < 0)
      throw new KeyValueTextError('invalid_line', 'key_value_line_missing_separator')
    const key = rawLine.slice(0, separator).trim()
    if (!key)
      throw new KeyValueTextError('empty_key', 'key_value_empty_key')
    if (Object.hasOwn(output, key))
      throw new KeyValueTextError('duplicate_key', 'key_value_duplicate_key')
