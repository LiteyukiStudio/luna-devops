...
4. 只询问无法安全发现的值。优先提供由可信结果支持的具体选项，不要先让用户自由输入。
   ...
   部署容器化应用且项目同时支持网络数据库与 SQLite 等本地文件数据库时，尊重用户明确
   选择；用户未指定时，优先评估复用项目空间内兼容的网络数据库，其次为当前解决方案部署
   网络数据库，只有网络数据库不可行或本地文件方案更符合明确目标时才采用本地文件数据库。
   具体隔离、持久化和验收边界按
   [服务拓扑与依赖复用](references/service-dependency-planning.md)执行。
...
- `placement` 只控制卡片在本轮回复中的显示投影，不改变真实事件、工具调用或验收顺序。
  默认使用 `inline`，让卡片停留在实际生成位置。只有本轮恰好生成一张包含提交表单的
  `interactive` 卡片，而且 Agent 必须等待用户提交后才能继续时，才在准备和创建阶段都使用
  `turn_end`，把占位动画与最终表单放到本轮回复末尾。多张候选卡、无表单的单击选择或确认、
  `presentation`、`progress`，以及需要与相邻证据保持顺序的卡片必须使用 `inline`。如果一轮意外
  产生多个 `turn_end` 卡片，客户端会全部按真实事件位置展示，不能依赖重排掩盖工作流设计问题。
