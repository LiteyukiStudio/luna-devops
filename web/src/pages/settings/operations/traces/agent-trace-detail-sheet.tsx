...
import { filterAgentTraceDisplaySpans, isAgentModelSpan, isCanonicalAgentToolSpan } from '@/pages/settings/operations/turns/agent-turn-timeline'
...
function SpanInspector({ span }: { span: AgentObservabilityTraceSpan }) {
  ...
  return (
    <div className="grid gap-4">
      ...
      <dl className="grid grid-cols-2 gap-3 text-sm [&_[data-slot=description-list-value]]:break-all">
        <DescriptionListItem label={t('operationsDashboardPage.duration')} value={formatDuration(span.durationMs)} />
        <DescriptionListItem label={t('operationsDashboardPage.status')} value={span.status} danger={span.status === 'error'} />
        <DescriptionListItem label={t('operationsDashboardPage.traceDetail.spanId')} value={span.spanId} mono />
        <DescriptionListItem label={t('operationsDashboardPage.traceDetail.parentSpan')} value={span.parentSpanId || '—'} mono />
      </dl>
