...
func (h *Handler) replaceScopedResourceProjectBindings(tx *gorm.DB, resourceType, resourceID string, projectIDs []string, defaultProjectIDs []string) error {
...
func (h *Handler) scopedResourceProjectIDMap(resourceType string, resourceIDs []string, ctx context.Context) map[string][]string {
	...
	var bindings []model.ScopedResourceProjectBinding
	if err := h.dbWithContext(ctx).Where("resource_type = ? and resource_id in ?", resourceType, resourceIDs).Order("project_id asc").Find(&bindings).Error; err != nil {
		return result
	}
	for _, binding := range bindings {
...
func (h *Handler) scopedResourceDefaultProjectIDMap(resourceType string, resourceIDs []string, ctx context.Context) map[string][]string {
	...
	var bindings []model.ScopedResourceProjectBinding
	if err := h.dbWithContext(ctx).Where("resource_type = ? and resource_id in ? and is_default = ?", resourceType, resourceIDs, true).Order("project_id asc").Find(&bindings).Error; err != nil {
		return result
	}
	for _, binding := range bindings {
		result[binding.ResourceID] = append(result[binding.ResourceID], binding.ProjectID)
...
func (h *Handler) canManageAllScopedProjects(ctx *gin.Context, user model.User, projectIDs []string) bool {
