...
func (h *Handlers) issueRuntimeTerminalTicket(
	...
	if h.rateLimiter != nil && h.rateLimiter.redis != nil {
		...
		err = h.rateLimiter.redis.Set(redisCtx, runtimeTerminalTicketKeyPrefix+hashToken(ticket), payload, runtimeTerminalTicketTTL).Err()
		cancel()
		if err == nil {
...
func (h *Handlers) consumeRuntimeTerminalTicket(ctx context.Context, ticket string) (runtimeTerminalTicketValue, bool, error) {
	...
	case strings.HasPrefix(ticket, "rtt_r_"):
		...
		redisCtx, cancel := context.WithTimeout(ctx, 500*time.Millisecond)
		raw, err := h.rateLimiter.redis.GetDel(redisCtx, runtimeTerminalTicketKeyPrefix+hashToken(ticket)).Bytes()
		cancel()
