...
export const createOptionsTool: ModelToolDefinition = {
  ...
  description: "当本轮回复确实存在清晰、可独立点选的后续操作时，展示 2～5 个简洁且符合当前上下文的快捷选项；任务正在等待表单提交、被批准/MFA 阻塞、或没有下一步时不要调用。选项会在输入框上方以单行胶囊按钮呈现，因此 label 必须是可独立理解的纯文本短语，不写 emoji、句号、解释、编号或重复前缀；中文通常不超过 18 个字，其他语言不超过 32 个字符。description 不会在快捷栏展示，只在确有调试价值时提供。若每个候选都有清晰且一致的视觉语义，可以使用 visual 增加扫读性；整组选项必须全部使用相同 visual.type，无法为所有选项找到准确图形时必须全部省略，不得为了装饰强行添加。emoji 使用直观字符；icon 只能使用 schema 允许的项目图标名；img 必须来自当前可信工具结果中的 HTTPS 图片，不得编造链接。动作必须匹配最直接的用户意图：send_message 只用于已知候选的单击选择、自然语言澄清或继续分析；request_tool 用于提出参数已就绪的已注册操作；navigate 只用于读取、浏览或明确打开已知页面。需要用户填写、选择、切换或组合任何结构化操作参数时，必须改用 create_interaction_cards 的 form；不得生成要求用户编辑消息、替换占位符或随后继续输入参数的选项。不得把必要的资源选择变成页面跳转，也不得用无关跳转打断待完成的决定。选择快捷选项作为本轮后续交互时只调用一次本工具，不得再生成重复交互卡片。每个选项相互独立；导航默认可重复，消息和受控工具操作只能执行一次。不得声称操作已经执行，不得编造路由、ID 或操作，也不得用空泛建议代替对当前需求的直接回应。",
