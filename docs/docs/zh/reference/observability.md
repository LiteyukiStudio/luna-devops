...
配置完成后进入“运营面板 → Agent 观测”，可以查看 Run 成功率、活跃 Run、首 Token P95、模型与依赖错误率、Token 吞吐、工具失败趋势、最近 Agent Run 和 Trace 关联失败日志。每轮用户消息通常产生一条 Trace，同一会话的多轮 Trace 通过 Conversation ID 关联。打开最近 Run 的“查看调用链”，可以使用默认完整展开的内嵌瀑布视图检查 Span 父子关系、并行阶段、耗时占比、错误节点，以及模型、工具、HTTP 和数据库调用；Span 树支持逐节点或整体折叠与展开。侧栏只展示经过白名单筛选的模型、Token、工具和错误属性，不展示 Prompt 或工具敏感参数。
