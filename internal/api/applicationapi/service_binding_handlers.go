...
func (h *Handlers) CheckServiceBinding(ctx *gin.Context) {
	...
	if result.Status == observation.StatusDeclared {
		...
		client, _, unavailableCode := h.host.KubernetesClientForDeploymentTargetObservation(project, targetTarget, ctx.Request.Context())
		if client == nil {
		...
		diagnostic, diagnosticErr := client.CheckServiceDependency(readContext, kubeprovider.ServiceDependencyCheckOptions{
			SourceNamespace: h.host.RuntimeProjectNamespace(project),
			TargetNamespace: h.host.RuntimeProjectNamespace(project),
			ServiceName:     h.host.DeploymentTargetResourceName(targetTarget),
