...
访问入口的默认域名后缀、外层访问协议、外层访问端口和 Gateway API 默认值在“运行集群”里维护。不同集群可以配置不同的网关域名后缀、GatewayClass 和共享 Gateway；部署配置绑定到哪个集群，默认域名、短前缀补全和控制台访问链接就按该集群配置生成。外层 CDN 或反向代理已经提供 HTTPS 时，可以把对应集群的外层访问协议改为 `https`；它只影响控制台展示和跳转链接，不会修改集群内部 listener，也不会触发证书申请。
