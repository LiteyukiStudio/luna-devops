...
镜像站用于拉取或推送镜像。常见选择包括 Harbor、Gitea Registry、DockerHub 和通用 OCI / Docker Registry。
...
平台管理员可以在工作负载页的 Pod 子行打开 Web Console，直接进入该 Pod 的交互式终端。该入口复用应用部署 Web Console 的终端组件，会校验 Pod 是否仍归属平台管理资源，并写入审计日志；如果站点开启敏感操作二次验证，打开前同样需要通过 `runtime_terminal` Step-up 校验。
