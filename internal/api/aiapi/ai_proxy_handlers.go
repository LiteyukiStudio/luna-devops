...
type aiProxyRoute struct {
	...
	stream   bool
	status   int
	validate func(*Handler, *gin.Context, model.User, []byte) bool
}
...
func (h *Handler) GetAICapabilities(ctx *gin.Context) {
...
func (h *Handler) ProxyAIRequest(ctx *gin.Context) {
...
func (h *Handler) attachAIModelSnapshot(ctx *gin.Context, userID string, body []byte) ([]byte, bool) {
...
func (h *Handler) aiActorFromSession(ctx *gin.Context) (aiagent.ActorContext, string, bool) {
	if actor, role, ok, configured := h.host.AIActorOverride(ctx); configured {
		return actor, role, ok
	}
	session, ok := h.host.CurrentSessionFromCookie(ctx)
	if !ok {
...
func (h *Handler) aiAccessAllowed(role string) bool {
	if !h.host.ConfigAvailable() {
		return false
	}
	mode := strings.TrimSpace(h.configValues([]string{aiAccessModeConfigKey})[aiAccessModeConfigKey])
	switch mode {
...
func (h *Handler) aiUnavailableReason() string {
	if !h.aiAssistantEnabled() {
		return "ai.disabled"
	}
	if h.host.AIAgent() == nil {
		return "ai.agent_unavailable"
	}
	return ""
}

func (h *Handler) aiAssistantEnabled() bool {
	if !h.host.AIDeploymentEnabled() || !h.host.ConfigAvailable() {
		return false
	}
	return strings.EqualFold(strings.TrimSpace(h.configValues([]string{aiAssistantEnabledConfigKey})[aiAssistantEnabledConfigKey]), "true")
}
...
func (h *Handler) readAIBody(ctx *gin.Context) ([]byte, bool) {
...
func validateCreateAIConversation(h *Handler, ctx *gin.Context, user model.User, body []byte) bool {
	var input struct {
	...
	if input.ProjectID != "" {
		if _, ok := h.host.FindProjectForCurrentUserByID(ctx, input.ProjectID); !ok {
			return false
		}
	}
	return true
...
func validateUpdateAIConversation(h *Handler, ctx *gin.Context, _ model.User, body []byte) bool {
	var input struct {
		Title   *string `json:"title"`
		ModelID *string `json:"modelId"`
	}
...
func validateEnabledAIModel(h *Handler, ctx *gin.Context, modelID string) bool {
...
func validateTurnInput(h *Handler, ctx *gin.Context, _ model.User, body []byte) bool {
	var input struct {
		ModelID string `json:"modelId"`
		Input   struct {
			Parts []struct {
...
func validateToolActionInput(_ *Handler, ctx *gin.Context, _ model.User, body []byte) bool {
	var input struct {
...
func validateRunInput(_ *Handler, ctx *gin.Context, _ model.User, body []byte) bool {
	var input struct {
		Text            string `json:"text"`
