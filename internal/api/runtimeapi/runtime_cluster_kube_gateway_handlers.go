...
const (
	maxRuntimeClusterKubeGatewayStatusBatch     = maxRuntimeClusterPressureBatch
	runtimeClusterKubeGatewayStatusConcurrency  = 6
	runtimeClusterKubeGatewayObservationTimeout = 8 * time.Second
)

type runtimeClusterKubeGatewayStatusResponse struct {
...
type runtimeClusterKubeGatewayStatusListResponse struct {
	Items []runtimeClusterKubeGatewayStatusResponse `json:"items"`
}

type runtimeClusterKubeGatewayObserver func(context.Context, model.RuntimeCluster) runtimeClusterKubeGatewayResponse
...
func (h *Handlers) enqueueKubectlGateway(ctx context.Context, clusterID string) error {
	return h.host.EnqueueKubectlGateway(ctx, strings.TrimSpace(clusterID))
}
...
func (h *Handlers) ObserveRuntimeClusterKubeGatewayStatus(ctx *gin.Context) {
	...
	clusterIDs, valid := runtimeClusterKubeGatewayStatusIDs(ctx.QueryArray("clusterId"))
	if !valid {
		writeErrorCode(ctx, http.StatusBadRequest, "runtime_cluster.kube_gateway_cluster_ids_invalid", "clusterId must contain between 1 and 100 unique runtime cluster identifiers")
		return
	}

	var clusters []model.RuntimeCluster
	if err := runtimecluster.ActiveScope(h.dbFor(ctx)).
		Where("id IN ? AND type IN ?", clusterIDs, []string{"kubernetes", "k3s"}).
	...
	items := observeRuntimeClusterKubeGatewayStatuses(
		ctx.Request.Context(),
		ordered,
		runtimeClusterKubeGatewayObservationTimeout,
		h.observeRuntimeClusterKubeGateway,
	)
	ctx.JSON(http.StatusOK, runtimeClusterKubeGatewayStatusListResponse{Items: items})
}

func observeRuntimeClusterKubeGatewayStatuses(
	ctx context.Context,
	clusters []model.RuntimeCluster,
	timeout time.Duration,
	observer runtimeClusterKubeGatewayObserver,
) []runtimeClusterKubeGatewayStatusResponse {
	items := make([]runtimeClusterKubeGatewayStatusResponse, len(clusters))
	guard := make(chan struct{}, runtimeClusterKubeGatewayStatusConcurrency)
	...
	for index := range clusters {
		...
		go func() {
			defer wait.Done()
			if ctx.Err() != nil {
				items[index] = unavailableRuntimeClusterKubeGatewayStatus(clusters[index])
				return
			}
			select {
			case guard <- struct{}{}:
			...
			observation := observer(probeCtx, clusters[index])
			...
			items[index] = runtimeClusterKubeGatewayStatusFromObservation(clusters[index], observation)
		}()
	}
	wait.Wait()
	return items
...
func runtimeClusterKubeGatewayStatusFromObservation(cluster model.RuntimeCluster, observed runtimeClusterKubeGatewayResponse) runtimeClusterKubeGatewayStatusResponse {
	checkedAt := time.Now().UTC()
	if observed.LastCheckedAt != nil {
		checkedAt = observed.LastCheckedAt.UTC()
	}
	response := runtimeClusterKubeGatewayStatusResponse{
		ClusterID: cluster.ID, Enabled: cluster.KubeGatewayEnabled, LastCheckedAt: checkedAt,
	}
...
func unavailableRuntimeClusterKubeGatewayStatus(cluster model.RuntimeCluster) runtimeClusterKubeGatewayStatusResponse {
	return runtimeClusterKubeGatewayStatusFromObservation(cluster, runtimeClusterKubeGatewayResponse{Status: "unavailable"})
}

func runtimeClusterKubeGatewayStatusIDs(raw []string) ([]string, bool) {
	return runtimeClusterPressureIDs(raw)
}
...
func (h *Handlers) observeRuntimeClusterKubeGateway(ctx context.Context, cluster model.RuntimeCluster) runtimeClusterKubeGatewayResponse {
	...
	if err != nil {
		rules = []runtimeClusterKubeGatewayRule{}
	}
...
func normalizeRuntimeClusterKubeGatewayRules(input []runtimeClusterKubeGatewayRule) []runtimeClusterKubeGatewayRule {
	result := append(make([]runtimeClusterKubeGatewayRule, 0, len(input)), input...)
