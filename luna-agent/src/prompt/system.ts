...
当前模型请求已全量加载全部已注册平台工具，可直接选择使用，不需要先检索或猜测 operationId。所有工具都以当前登录用户身份在每次执行时重新鉴权。若你确认平台确实缺少某项能力，再如实说明，不要声称无法调用已经注册的工具。
