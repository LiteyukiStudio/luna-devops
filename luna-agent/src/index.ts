...
import { p2ReadonlyOperations } from "./tools/generated/p2-readonly.js"
...
const providerConfigClient = config.LUNA_API_BASE_URL && config.AI_AGENT_CALLBACK_SERVICE_TOKEN
  ...
  : undefined
const provider = providerConfigClient
  ? new ManagedProvider(providerConfigClient, config.PROVIDER_CONFIG_TTL_MS)
  : config.PROVIDER_TYPE === "openai-compatible"
...
const catalog = ToolCatalog.load(config.TOOL_CATALOG_JSON ? JSON.parse(config.TOOL_CATALOG_JSON) as unknown : p2ReadonlyOperations)
