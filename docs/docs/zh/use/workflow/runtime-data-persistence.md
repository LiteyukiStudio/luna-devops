...
导入会把归档直接写入临时 Transfer Pod 挂载的目标卷，不经过对象存储、浏览器预哈希或平台本地完整暂存。API 与 Transfer Pod 会在同一次流传输中独立计算摘要并核对完整性。连接中断后不能续传，也不能在原目标卷上重试；请删除失败的导入卷并创建一次全新导入，避免残留数据与新归档混合。只有 Transfer 和数据卷都进入成功/就绪终态后，才可以认为恢复完成。
...
导出内容从 Transfer Pod 直接流式下载，不保留可重复下载的服务端归档，也不支持断点续传。下载授权是绑定当前用户、会话和 Transfer 的短期一次性票据。只有下载完成且 Transfer 显示“成功”才表示本次备份完成；连接中断后需要重新导出。
