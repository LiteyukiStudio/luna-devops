...
func (h *Handler) registryForCurrentUser(ctx *gin.Context) (model.User, model.ArtifactRegistry, bool) {
...
func (h *Handler) findAccessibleRegistry(ctx *gin.Context, user model.User, registryID string) (model.ArtifactRegistry, bool) {
	...
	registry.ProjectIDs = h.scopedResourceProjectIDs(scopedResourceArtifactRegistry, registry.ID, ctx.Request.Context())
	return registry, true
}

func (h *Handler) canUseRegistry(ctx *gin.Context, user model.User, registry model.ArtifactRegistry) bool {
...
func (h *Handler) canManageRegistry(ctx *gin.Context, user model.User, registry model.ArtifactRegistry) bool {
...
func (h *Handler) canManageRegistryCredential(ctx *gin.Context, user model.User, registry model.ArtifactRegistry, credential model.RegistryCredential) bool {
...
func (h *Handler) defaultRegistryFor(userID, projectID string, ctx context.Context) (model.ArtifactRegistry, bool) {
	var projectRegistry model.ArtifactRegistry
	projectDefault := h.dbWithContext(ctx).
	...
	if projectDefault.Error == nil {
		projectRegistry.ProjectIDs = h.scopedResourceProjectIDs(scopedResourceArtifactRegistry, projectRegistry.ID, ctx)
		projectRegistry.DefaultProjectIDs = h.scopedResourceDefaultProjectIDMap(scopedResourceArtifactRegistry, []string{projectRegistry.ID}, ctx)[projectRegistry.ID]
	...
	for _, candidate := range candidates {
		var registry model.ArtifactRegistry
		err := h.dbWithContext(ctx).First(&registry, "scope = ? and owner_ref = ? and is_default = ?", candidate.scope, candidate.owner, true).Error
		if err == nil {
			return registry, true
		}
	}
