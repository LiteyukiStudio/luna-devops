...
- OpenAPI 与手写工具组成完整权威目录，但每次模型请求只下发与当前页面、用户目标和已完成步骤
  相关的聚焦子集，默认不超过 24 个，避免大量相似定义挤占上下文并降低选择准确率。
- `search_tools` 是 Harness 内部发现工具。模型准备声明“没有工具”或“无法执行”前，必须先用
  具体的业务目标、资源和期望结果检索；检索命中的定义在同一 Run 下一模型步骤动态加入，检索本身
  不代表授权或执行。相同检索不会重复扩张工具集，审批/MFA 恢复后保留已使用的工具定义。
- 工具描述必须说明用途、禁用场景、前置条件、主要参数和成功回读方式。OpenAPI 的 summary 与
  description 只作为检索语义提示；模型可见说明由 Agent 统一生成中文边界，避免泛化为“调用某 API”。
- 检索质量用固定场景集回归：常见目标的正确工具必须进入前 8 个结果，结果数量受限；新增或变更
  工具时同步更新意图提示与评测，不以扩大默认工具集掩盖召回问题。
