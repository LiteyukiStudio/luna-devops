...
import { AgentTokenUsageStrip } from '@/pages/settings/operations/agent-token-usage'
import { agentModelOutput, agentSpanContentSections, agentSpanMessageMarkdown, agentSpanMessages, formatSpanJSON, isAgentSpanContentAttribute } from '@/pages/settings/operations/traces/agent-span-content'
      ...
      <div className="mb-4 overflow-hidden rounded-container bg-surface-raised">
        ...
        {expanded && (
          <div className="grid gap-3 border-t border-border px-4 py-3">
            ...
            <dl className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4 [&_[data-slot=description-list-value]]:break-all">
              <DescriptionListItem label={t('operationsDashboardPage.status')} value={<StatusBadge tone={spanStatusTone(span.status)}>{spanStatusLabel(span.status, t)}</StatusBadge>} />
              <DescriptionListItem label={t('operationsDashboardPage.duration')} value={formatDuration(span.durationMs)} />
              <DescriptionListItem label={t('operationsDashboardPage.turnDetail.spanId')} value={span.spanId} mono />
              <DescriptionListItem label={t('operationsDashboardPage.turnDetail.parentSpan')} value={span.parentSpanId || '—'} mono />
            </dl>
            {attributes.length > 0 && (
