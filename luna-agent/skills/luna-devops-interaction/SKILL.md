...
- 检索会保留本 Run 已使用工具、等待中的操作和写后验收工具。审批、MFA 或异步回读期间不要检索无关写工具。
...
- 快捷选项由完成回复后的独立预测阶段生成，主任务模型不调用 `create_options`，更不能用快捷选项代替工具检索、参数收集或业务执行。
...
- `review_tool_action`：只用于执行写操作前核对已确认的目标、参数、影响和风险；它不替代平台批准或 MFA。
