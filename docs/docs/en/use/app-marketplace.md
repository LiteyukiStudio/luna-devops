...
When Gateway Traffic Probe is not installed, the billing page shows gateway traffic as unavailable and guides platform administrators to install it from the marketplace. The system component installation record stores only the installation workflow and resource references. Luna reads the probe workload from the target Kubernetes cluster for every status request and does not cache online state. A ready workload without a valid positive traffic window is shown as **Waiting for report**; traffic spend appears after the first valid report, and a disconnected cluster is shown as **Unavailable**.
