...
每个阶段都必须有最大重试次数和明确终态。禁止模型无限循环调用列表、日志或
状态接口。
...
- `proposed` 时完成参数 Schema 校验、风险计算和 `arguments_hash` 固化。
...
不能把全部平台接口一次性暴露给模型。采用两阶段发现：

1. 根据用户意图和页面上下文选择领域。
2. 从领域目录中加载 5 到 15 个最相关工具。

工具目录以 OpenAPI operation 为事实源，并补充以下元数据：
...
x-luna-agent:
  ...
  requiredScopes:
    - deployment:write
  approval: always
  stepUpPurpose: deployment_update
  idempotent: true
  supportsDryRun: true
  sensitiveFields:
    - secret
...
1. OpenAPI operation 显式声明 `x-luna-agent` 才能进入候选集合。
2. CI 从 OpenAPI 生成 TypeScript Client、Tool JSON Schema、风险目录和 Scope 映射。
3. CI 校验 operation ID 唯一、Scope 存在、风险等级合法、写操作具有幂等策略。
4. 含 Secret、任意 URL、文件上传、终端或自由表达式的 operation 默认拒绝生成，
   必须增加专用策略适配器。
5. Agent 启动时校验工具目录摘要与自身构建摘要；摘要不同则不进入 Ready。

OpenAPI 中未声明元数据的 operation 默认不可用，禁止根据 URL 或 HTTP Method
自动推断为低风险工具。工具参数必须使用 JSON Schema 严格模式：
...
- Provider fallback 只能继续同一 Run，必须从持久 Tool Call 记录恢复，不能忘记
  已经发生的副作用后重新规划。
- 达到循环或预算上限后以 `ai.limit_exceeded` 失败，并保留已完成操作的摘要。
...
```text
用户当前权限
∩ 当前登录或 OAuth Grant Scope
∩ 当前会话项目边界
∩ 工具要求的 Scope
```

委托令牌不能直接作为业务授权结论。每次工具调用仍由 Luna API 查询当前用户、
成员角色、资源归属、Scope 和 MFA assertion。用户被移出项目、角色变化、Token
吊销或 MFA 过期后，下一次工具调用必须立即失败。

实现约束：

- 有效期默认 60 秒，最长 5 分钟；Agent 按需换取，不为整个会话签发长 Token。
- Token 只能访问生成工具目录中的 operation ID，不能访问 `/internal/*`。
- 业务请求的路由 operation ID 必须与 Token 中 `operationId` 完全一致；不能用一枚
  Token 连续调用多个 operation。
- `jti` 与 Run、用户、Session 和 OAuth Grant 绑定，可在 Redis 中短时吊销。
- 浏览器 Session 退出、OAuth Grant 吊销、用户禁用或 Run 终止时立即吊销未过期
  委托令牌。
...
5. 未完成 Run 同时返回各自的 `eventCursors`，浏览器用 SSE 补齐快照后的增量。
...
- `items` 始终按 `timelineIndex` 排序；工具真实完成顺序只通过 SSE
  `eventSequence` 呈现。
...
- Timeline 响应必须在一致性快照中读取，并返回快照提交时每个非终态 Run 的事件
  高水位。浏览器先渲染完整快照，再分别从对应 `after` 订阅 SSE，避免刷新期间出现
  缺口或重复。
...
- 一个完成的语义 Item、对应 Content Part 和 `item.completed` Run Event 必须在同一
  数据库事务提交，随后通过事务 Outbox 或等价机制发布 Redis live event。
- SSE 可以先发送暂态 Delta，但客户端收到 `item.completed` 后必须以持久 Item 为准。
...
- 全量快照加载完成前显示结构化 skeleton；加载成功后再合并 SSE 增量。相同
  `eventId`、Item ID 和 Content Part ID 必须幂等去重。
...
- Timeline 首次快照、SSE 增量和重连重放必须复用同一个 reducer，并按 `eventId`、
  Item ID、Part ID、Tool Call ID 幂等合并，避免工具卡片重复或展开状态丢失。
...
- 回答中的 Markdown 链接用于直接引用正在讨论的资源；每个正常完成的 Turn 必须
...
- 动作默认需要用户点击。无副作用且不改变用户当前工作上下文的 `refresh_query`
  ...
  当前工作上下文的自动路由工具，只能在用户明确要求跳转或目标唯一且立即跳转确有
  必要时调用；前端只消费实时 SSE 中该 Tool Call 的完成事件，并按 Tool Call ID
  去重，历史 Timeline 与重连重放不能再次触发。
- 点击 Tool Call 的“打开发布详情”等动作后，前端先验证当前 Page Context 与
  registry version，再切换路由或 Tab；成功后在工具卡片显示“已打开”，失败时保持
  当前页面并显示可理解提示。
- 页面路由变化不会隐式改变会话绑定的项目空间。新页面上下文作为后续 Turn 输入，
  当前 Run 仍使用创建时固化的 Actor Context 与资源边界。
...
不能因为页面上下文声称某资源可见就跳过业务 API 查询。会话切换项目时必须显式
更新 `projectId`，不能自动继承旧项目 ID。
...
调用平台现有 `/api/v1/*` 业务路由。业务鉴权中间件必须校验实际路由的 OpenAPI
operation ID 与 Token Claim 一致，再进入现有 Scope、RBAC、MFA 和资源归属检查。
...
- `system-v3` 在固定安全提示后加载 `luna-devops-navigation` Skill。Skill 只维护
...
- 内建 `create_options` 是纯 UI 工具，不访问业务数据。输入包含标题、2-5 个选项及
  ...
  必须产生一次。主回复遗漏时，Agent 使用指定 `tool_choice` 进行独立结构化预测，
  Provider 仍不兼容时才使用路由感知的安全兜底。`request_tool` 只能转成新的用户
...
AND default model has an active pricing version
...
| `AI_AGENT_ADDR` | 空 | Agent 内部地址，生产环境无默认值 |
| `AI_AGENT_TIMEOUT` | `10s` | 非流式内部请求超时 |
| `AI_AGENT_CA_FILE` | 空 | mTLS CA |
| `AI_AGENT_CERT_FILE` | 空 | mTLS Client Cert |
| `AI_AGENT_KEY_FILE` | 空 | mTLS Client Key |
| `AI_TRUST_SIGNING_KEYRING_FILE` | 空 | API 侧四类信任材料的版本化签名私钥 Keyring |
| `AI_AGENT_SERVICE_VERIFY_KEYRING_FILE` | 空 | Agent 服务 JWT 的验证公钥 Keyring |

`luna-agent`：

| 环境变量 | 默认值 | 说明 |
| --- | --- | --- |
| `AGENT_LISTEN_ADDR` | `:8080` | 内部 HTTP |
| `AGENT_METRICS_ADDR` | `:9092` | 独立 metrics listener |
| `DATABASE_URL` | 空 | 仅具有 `ai` schema 权限的数据库角色 |
| `REDIS_ADDR` | 空 | 使用项目统一 `redis://` URL |
| `LUNA_API_ADDR` | 空 | Luna API 内部地址 |
| `LUNA_API_CA_FILE` | 空 | 调用 API 的内部 CA |
| `LUNA_API_CERT_FILE` | 空 | 调用 API 的 mTLS Client Cert |
| `LUNA_API_KEY_FILE` | 空 | 调用 API 的 mTLS Client Key |
| `AI_TRUST_VERIFY_KEYRING_FILE` | 空 | API 服务 JWT 与 Actor Context 的验证公钥 Keyring |
| `AI_AGENT_SERVICE_SIGNING_KEYRING_FILE` | 空 | Agent 服务 JWT 的签名私钥 Keyring |
| `AI_GRANT_ENCRYPTION_KEYRING_FILE` | 空 | Run Actor Grant 的版本化数据加密密钥 |
| `AI_RUN_LEASE_SECONDS` | `30` | Run 租约 |
| `AI_RUN_HEARTBEAT_SECONDS` | `10` | 租约续期 |
| `AI_DRAIN_TIMEOUT` | `20s` | 优雅退出时间 |
...
配置，只允许平台管理员填写 `https` URL，并受出站 allowlist 约束。
...
| `ai.provider.type` | 空 | P0 仅允许 `openai-compatible` |
| `ai.provider.base_url` | 空 | 必须为管理员 allowlist 内的 `https` URL |
| `ai.provider.api_key` | 未设置 | Secret 字段，编辑留空表示不修改 |
| `ai.provider.default_model` | 空 | 必须通过 Provider 模型探测或管理员确认 |
| `ai.provider.fallback_model` | 空 | 可空，不能与默认模型形成无限切换 |
| `ai.provider.model_pricing` | `[]` | 版本化模型输入/输出单价；生产开放前必填 |
...
```json
{
  ...
  "promptVersions": ["system-v1", "system-v2", "system-v3"]
}
```
...
- 在现有平台 OpenAPI operation 上增加显式 `x-luna-agent` 元数据，首批只开放
  只读 operation。
...
- 扩展 OpenAPI Agent 元数据和工具风险目录；未显式声明的写 operation 继续不可用。
...
- 平台 OpenAPI 中所有 `x-luna-agent` operation 都能解析成有效工具。
- 公开、内部和工具错误都使用稳定错误码。
