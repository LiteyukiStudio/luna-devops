...
配置完成后进入“运营面板 → Agent 观测”。页面顶部按所选 1、6 或 24 小时周期汇总输入 Token、输出 Token、工具调用、对话轮数量、终态轮次成功率和执行耗时 P95；下方按“从一次用户输入到 Agent 输出结束”为一轮，提供跨用户搜索和服务端分页。点击任意轮次会从右侧打开详情，不再提供会话级观测视图。
