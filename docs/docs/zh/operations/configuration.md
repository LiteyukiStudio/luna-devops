...
开启全局策略后，Web Console、运行命令、数据导出、登录密码修改、密钥和镜像站凭据写入、kubeconfig 更新、认证源更新、平台管理员账号变更及安全策略修改会检查当前浏览器会话与具体 purpose 的 Step-up assertion。权限边界明确的全局管理接口通过“认证、平台管理员授权、Step-up MFA”路由中间件声明保护；依赖项目角色、资源归属或请求内容的操作仍在完成业务授权后调用同一 Step-up guard，避免二次验证替代资源权限判断。未通过时 API 返回 `mfa_required`，控制台弹出统一验证码/恢复码 Dialog，并在验证成功后自动重试原操作。断言按用户、session 和 purpose 存在共享数据库中；成功执行同类敏感操作会刷新空闲时间，但不会延长绝对有效期。个人令牌不能完成 MFA，也不能替代这些交互式会话检查。
