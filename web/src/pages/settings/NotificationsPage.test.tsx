...
describe('notification rule scope', () => {
  ...
  it('defaults to project scope and requires an explicit all selection', async () => {
    ...
    const name = screen.getByText('Name', { selector: 'label' }).parentElement?.querySelector('input')
    const scope = screen.getByText('Notification scope', { selector: 'label' }).parentElement?.querySelector('select')
  ...
  it('blocks malformed advanced filter JSON instead of falling back to an empty filter', async () => {
    ...
    const filter = screen.getByText('Advanced filter JSON', { selector: 'label' }).parentElement?.querySelector('textarea')
