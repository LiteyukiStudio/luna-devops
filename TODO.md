...
## 2026-09-02 kubectl 网关配置弹窗崩溃修复

- [x] 修复空额外资源规则被后端序列化为 `null`、前端直接调用数组方法导致配置弹窗崩溃的问题。
- [x] 统一 `extraResourceRules` 与 `subresources` 的 OpenAPI、Go 响应和前端运行时数组契约，并兼容历史异常载荷。
- [x] 完成 Go、Web、OpenAPI、前端质量门禁与真实点击路径浏览器回归。

## 2026-09-01 集群管理行操作与网关状态收口

- [x] 将运行集群的测试、kubectl 网关、编辑与删除操作统一收进每行三点菜单，并把网关实时状态作为滚动区 Badge 展示。
- [x] 保持网关状态由后端按当前页批量实时观察，补齐 OpenAPI、前端类型、权限边界与契约测试。
- [x] 同步中英文基础资源文档，并完成 Go、Web、文档构建及桌面端/移动端浏览器验收。

## 2026-09-01 kubectl 凭据页体验收口

- [x] 修复状态筛选外层占满工具栏造成的错位，并让搜索和状态筛选在桌面同行、移动端纵向排列。
- [x] 区分加载、失败、首次为空和筛选为空状态，补充明确下一步、清除筛选与移动端关键列。
- [x] 同步五语言文案与中英文 kubectl 文档，并完成针对性测试、前端质量门禁和浏览器验收。

## 2026-08-31 账号页 Tab 操作收口

- [x] 将 Access Token 与 OAuth 应用 Tab 的创建按钮提升到 `ContentTabs.tools`，删除内容区独占操作行，并完成前端回归与质量门禁。

## 2026-08-31 后端 API 目录按领域分层

- [x] 将 `internal/api` 的扁平 Go 文件按真实业务领域拆分，领域 Handler、DTO 与测试共置，根包只保留装配、全局中间件、路由组合、静态资源和跨领域集成契约。
- [x] 同步启动装配、跨包窄接口、递归静态检查、CI 测试范围和内部文档路径，保持路由、OpenAPI、Scope、权限、审计、响应与可观测行为不变。
- [x] 完成旧路径和反向依赖残留检查、格式化、全量 Go 测试、Go vet 与文档构建。

## 2026-08-31 看板固定相关范围

- [x] 移除看板“与我相关 / 全部”选择器，将页面查询与下游链接固定为 `visibility=related`，并同步前端测试和中英文用户文档后完成前端与文档门禁。
...
- [x] 删除单场景透传包装与死组件，修正跨页面私有模块依赖，并将运行终端、项目数据卷等真实跨领域能力迁入公共边界。
- [x] 完成全量前端测试、i18n、lint、单例依赖、生产构建，并在桌面端与移动端验收未登录入口的响应式布局和控制台错误。
- [ ] 使用有效登录会话补充应用内公共组件与应用市场安装弹窗的桌面端、移动端交互验收；当前应用内浏览器和 Edge 会话均停留在登录页。

## 2026-08-30 前端页面目录按子领域分层

- [x] 将 `applications`、`settings`、`projects`、`billing`、`clusters` 五个扁平过密的页面领域按真实子领域继续分层，测试与实现保持共置。
- [x] 同步路由懒加载、跨页面引用、动态模块导出契约和 i18n 精确路径白名单，不改变页面行为。
- [x] 完成旧路径残留检查、全量前端测试、lint、单例依赖检查与生产构建。

## 2026-08-29 应用市场就地创建数据卷

- [x] 在需要持久化的模板安装弹窗中提供空白卷快捷创建，锁定当前项目空间、运行集群和模板卷模式，并在创建受理后自动选中。
- [x] 保持数据卷创建与模板安装的权限、审计和异步任务边界，修正可挂载卷契约说明，并覆盖创建后立即安装及任务投递失败后的同键幂等重试。
- [x] 同步中英文应用市场文档，完成前端、Go 和文档自动化验收。
- [ ] 使用有效登录会话补充桌面端与移动端浏览器交互验收；当前本地浏览器会话已过期。
