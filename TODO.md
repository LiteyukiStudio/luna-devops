...
- [x] 将旧 `docs/` 内容迁移到 `docs-legacy/`，并使用 Rspress 建立支持中英双语、响应式和多主题的文档站。
...
- [ ] 后续评估全局项目空间选择器：作为项目空间隔离上下文入口，不在当前阶段顶部栏落地。
...
- [ ] 计费主体统一为项目空间：每个项目空间拥有一个 credits 钱包，所有构建、运行、访问、存储等消耗都归属到项目空间；个人项目空间也走同一套模型。
- [ ] 全平台只使用 `credits` 作为内部货币，底层金额使用高精度 decimal 存储并支持小数，禁止使用 `float32/float64` 表示金额；展示单位、充值包和兑换关系后续由站点管理员在站点设置中配置。
- [ ] 费用相关写操作必须使用数据库事务保证原子性：账本流水、钱包余额、用量结算状态和补偿/退款流水必须在同一个事务内完成，任一步失败都整体回滚。
- [ ] 建立 append-only 账本：所有扣费、充值、赠送、补偿和退款都写入不可变流水，历史流水不修改；修正通过反向流水完成。
- [ ] 建立用量记录与账本分离模型：先记录原始用量，再按当时的计费规则快照生成 credits 流水，避免后续调价影响历史账单。
- [ ] 计费采用事件 + 周期采样 + 批量聚合：构建完成按事件结算，容器运行和存储按分钟采样并按小时聚合，访问次数按时间窗口聚合后再生成账本流水，不在请求路径逐次扣费。
- [ ] 构建 Job 计费：无论成功、失败、取消还是超时，只要 Kubernetes Job 已实际开始运行就按资源占用计费；排队未开始不计费，平台内部错误可生成补偿流水。
- [ ] 构建 Job 粗略计量系数先按 decimal 估算：`1 vCPU-minute = 10 credits`，`1 GiB-memory-minute = 2 credits`，最小计费粒度 1 分钟；最终系数不写死，放入站点配置或计费规则表。
- [ ] 容器运行计费：按项目空间内运行中工作负载的 `replicas * cpu_request * duration`、`replicas * memory_request * duration`、数据卷容量和访问次数累计 credits。
- [ ] 容器运行粗略计量系数先按 decimal 估算：`1 vCPU-hour = 30 credits`，`1 GiB-memory-hour = 6 credits`，`1 GiB-storage-day = 1 credit`，`1000 次访问 = 1 credit`；MVP 允许按分钟采样、按小时聚合。
- [ ] 访问计费先按入口请求次数聚合，不在第一版区分公网流量、带宽、地域和 TLS；后续需要时再增加 `egress_gib`、`cdn_request` 等 meter。
- [ ] 存储计费按声明容量计费，不按实际使用量计费；数据卷保留期间持续计费，导出数据卷可作为一次性操作账单项。
- [ ] 站点管理员可配置计费规则启停、credits 单位展示名、充值包、免费额度、项目空间欠费宽限期和是否允许欠费继续运行。
- [ ] 余额不足风控：禁止新构建、新发布、扩容和新增数据卷；运行中资源先进入欠费/宽限状态，不默认立即删除数据，避免误伤用户业务。
- [ ] 前端提供项目空间账单视图：展示余额、近期待扣用量、构建/运行/访问/存储分类消耗、账本流水和低余额提示。
...
- [x] 支持 HTTP-only 访问开关。
- [x] 新增站点级访问链接协议策略：`gateway.publicScheme` 控制控制台访问入口链接显示为 `http` 或 `https`，用于外层 CDN/反代已提供 HTTPS 但平台不申请集群内证书的场景。
