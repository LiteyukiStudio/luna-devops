...
describe("Agent prompt-cache HTML report", () => {
  it("renders escaped, self-contained, responsive light/dark and print markup", async () => {
    ...
    expect(html).toMatch(/\.main\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)/)
    expect(html).toMatch(/\.main\s*>\s*\*\s*\{[^}]*min-width:\s*0/)
    expect(html).toMatch(/\.scenario-stack\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)/)
    expect(html).toMatch(/\.table-region\s*\{[^}]*width:\s*100%[^}]*min-width:\s*0[^}]*max-width:\s*100%[^}]*overflow-x:\s*auto/)
    expect(html).toMatch(/\.raw-data pre\s*\{[^}]*width:\s*100%[^}]*min-width:\s*0[^}]*max-width:\s*100%[^}]*overflow:\s*auto/)
