...
func (h *Handlers) StartOIDC(ctx *gin.Context) {
	provider, ok := h.enabledAuthProvider(ctx.Param("providerId"), ctx.Request.Context())
	if !ok {
	...
	oauthConfig := h.oauth2Config(ctx.Request.Context(), provider, oidcProvider)
...
func (h *Handlers) CompleteOIDC(ctx *gin.Context) {
	...
	if err != nil {
		h.auditWithContext("", "oidc.callback", "oidc_state", false, "state missing or expired", ctx.Request.Context())
	...
	provider, ok := h.enabledAuthProvider(authState.ProviderID, ctx.Request.Context())
	if !ok {
		h.auditWithContext(authState.UserID, "oidc.callback", authState.ProviderID, false, "provider disabled", ctx.Request.Context())
	...
	oidcProvider, err := oidc.NewProvider(egressCtx, provider.IssuerURL)
	if err != nil {
		h.auditWithContext(authState.UserID, "oidc.callback", provider.ID, false, err.Error(), ctx.Request.Context())
	...
	if err != nil {
		h.auditWithContext(authState.UserID, "oidc.callback", provider.ID, false, "code exchange failed", ctx.Request.Context())
	...
	if !ok {
		h.auditWithContext(authState.UserID, "oidc.callback", provider.ID, false, "id_token missing", ctx.Request.Context())
	...
	if err != nil {
		h.auditWithContext(authState.UserID, "oidc.callback", provider.ID, false, "id_token verify failed", ctx.Request.Context())
		h.redirectAuthError(ctx, "oidc_token_invalid")
		return
	}
	if idToken.Nonce != authState.Nonce {
		h.auditWithContext(authState.UserID, "oidc.callback", provider.ID, false, "nonce mismatch", ctx.Request.Context())
		h.redirectAuthError(ctx, "oidc_state_invalid")
		return
	}
	...
	user, err := h.findOrCreateOIDCUser(provider, claims, ctx.Request.Context())
	if err != nil {
		h.auditWithContext("", "oidc.login", provider.ID, false, err.Error(), ctx.Request.Context())
	...
	h.auditWithContext(user.ID, "oidc.login", provider.ID, true, "login succeeded", ctx.Request.Context())
...
func (h *Handlers) completeOIDCBind(ctx *gin.Context, authState oidcAuthStateValue, provider model.AuthProvider, claims oidcIdentityClaims) {
	var user model.User
	if err := h.dbFor(ctx).First(&user, "id = ? and disabled = ?", authState.UserID, false).Error; err != nil {
		h.auditWithContext(authState.UserID, "oidc.bind", provider.ID, false, "user not found", ctx.Request.Context())
		h.redirectAuthError(ctx, "auth_forbidden")
		return
	}

	_, err := h.bindExternalIdentityToUser(user, provider, claims, ctx.Request.Context())
	if err != nil {
		h.auditWithContext(user.ID, "oidc.bind", provider.ID, false, err.Error(), ctx.Request.Context())
		h.redirectAuthError(ctx, "oidc_bind_failed")
		return
	}

	h.auditWithContext(user.ID, "oidc.bind", provider.ID, true, "identity bound", ctx.Request.Context())
	ctx.Redirect(http.StatusFound, "/settings/security")
}

func (h *Handlers) oauth2Config(ctx context.Context, provider model.AuthProvider, oidcProvider *oidc.Provider) oauth2.Config {
