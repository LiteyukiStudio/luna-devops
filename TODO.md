...
- [x] 将五国语言资源改为当前语言按需加载，将浏览器 OTel SDK/Exporter/Processor 移出匿名首屏关键路径，同时保留 W3C Trace Context 与同源 Relay 契约。
- [x] 将全局页面/列表入场动画收敛为 CSS，登录插画改用同源静态资源，避免首屏引入 Motion 运行时和跨域关键图片请求。
- [x] 完成 Web/Docs/Go 全量门禁与生产预览的桌面、移动端浏览器验收；首屏关键 JS（含当前中文）由 2,099,198 B / 636,774 B gzip 降至 817,926 B / 263,863 B gzip，分别下降 61.0% / 58.6%，最大关键 Chunk 由 1,448,015 B 降至 249,986 B。
- [x] 为生产静态资源生成 Brotli/gzip 并按 `Accept-Encoding` 协商，剔除仅供仓库说明使用的 4.4 MB 图片；验证移动端继续不请求桌面登录插画。
- [x] 将匿名路由与登录后布局、核心文案与功能文案、项目空间 Tab 组件进一步分包，按当前页面启用查询。
- [x] 将空闲看板和部署观察轮询退避到 30 秒，活动构建、发布和证书流程继续保持 2 秒刷新；完成优化前后 benchmark、桌面/移动端验收和 HTML 报告。以 `bf591b89` 为基线，登录关键 JS + CSS 实际传输由 2,421,730 B 降至 201,564 B（-91.7%），报告见 `reports/frontend-performance-2026-08-21.html`。

## 2026-08-21 Agent 条件必填参数契约修复
...
- [x] 将工具发现升级为字段加权 BM25F；非空检索自动加载候选，精确详情只返回紧凑语义，重复请求复用真实缓存结果。
- [x] 为 Run 持久化最多 16 项、按最近使用淘汰的工具选择，并让模型轮次、审批、输入恢复与实际调用共享同一集合。
- [x] 为 Catalog 增加 digest 快照和原子热更新，新旧 Run 隔离目录版本；系统 Prompt 使用稳定核心前缀和独立动态参考消息。
- [x] 完成全量 Agent/Go/Web/Docs 门禁、真实浏览器场景、全部 AI operation 检索覆盖与代表性执行回读，并确认 GitHub Actions 通过。

## 2026-08-21 发版前数据库迁移压平

- [x] 将 `000001–000067` 历史迁移压平为 `000067_baseline`，保留 `000068+` 的配额、传输并发、AI 模型和运行时数据转换升级链。
- [x] 使用临时 PostgreSQL 对比旧全量链与新 baseline 链的最终 Schema 和初始系统数据，确认差异为零。
- [x] 删除只服务于已放弃旧版本升级路径的测试，将最终字段、废弃字段、关键索引和模型覆盖收敛到空库迁移验收。
- [x] 明确低于版本 67 的开发数据库必须销毁重建，不通过 `migrate.Force` 认领旧 Schema。
...
- [x] 推送 `main` 并确认 GitHub Actions 全部通过。
...
- [x] 将重新部署说明收敛为操作栏同排黄色文本，补齐 ServiceAccount、ConfigMap 对象和部署阶段 Hook 的变化比较。
- [x] 修复 Key/Value 编辑器按异常 message 拼接不存在 i18n key 的问题，统一按稳定错误码显示中英文校验文案。

## 2026-08-17 部署运行状态统计口径

- [x] 将运行状态和副本数收敛到同一次部署目标 Kubernetes 观察，并在部署页按工作流状态独立展示 Release 结果。
- [x] 将零副本运行时标记为中性“已缩容”，未满足 ready/available 副本数时降级为部署中，观测不可用时不显示默认 `0/0`。
- [x] 同步 OpenAPI、Agent 工具说明、中英文使用文档与前后端回归测试。

## 2026-08-17 运行时环境变量显式模式

- [x] 从部署目标、运行配置集和 ConfigMap 引用保存链路移除敏感键名与值内容的启发式阻断，用户显式选择 `public` 后允许保存任意合法键值。
- [x] 删除旧敏感语义阻断与同键双模式冲突错误；保留 public/secret 显式类型、Secret 加密存储及不回显边界，同名时由密钥覆盖普通值。
- [x] 更新 OpenAPI、中英文使用文档与回归测试，明确 public 值可被有权客户端读取，历史只读明文审计不参与写入校验。

## 2026-08-17 部署配置表单交互回归

- [x] 移除运行时密钥编辑器嵌套表单，将添加与替换收敛为显式按钮操作，避免误提交并关闭外层部署配置 Dialog。
- [x] 将数据卷编辑器改为带字段标签的响应式双列卡片，并允许数据卷选择器操作区换行，消除配置窗口横向溢出。
- [x] 在真实浏览器中逐段展开部署配置全部区域，验收数据卷桌面端与移动端布局、Dialog 边界和控制台；数据卷无横向滚动或越界元素。
- [x] 将密钥编辑器嵌入真实外层部署表单进行交互回归，覆盖添加与替换按钮不会提交或关闭外层 Dialog。

## 2026-08-17 部署配置阶段与安全错误边界

- [x] 将普通创建、模板安装和部署配置 preview/import 收敛到同一公开阶段校验，支持 `production` 归一化并要求非法历史阶段通过合法 override 恢复。
- [x] 建立部署配置稳定错误目录，移除 JSON、数据库、Secret、资源规格和模板解析链的原始错误回显；同步 OpenAPI 与中英文 i18n。
- [x] import 事务返回实际 HookBinding/VolumeMount 供 201 响应使用，失败路径保持零 DeploymentTarget、Secret、构建环境和挂载副作用。
- [x] 审计记录保留资源可追溯 ID，失败遥测只记录稳定 action/resource type/error type；候选翻页保持无 AuditLog。
...
- [x] 将显式引用映射改为按资源类型和目标作用域直接校验，不再依赖截断候选；自动匹配按全量兼容结果判定唯一性。
- [x] 为七类部署依赖增加真实总数、搜索、白名单排序和分页候选接口，Web 导入映射支持搜索、排序和翻页且不允许手填 ID。
- [x] 同步 OpenAPI、前端类型/API Client、中英文文档和 PostgreSQL/前端契约测试；候选响应禁用缓存，不把高频翻页写入 AuditLog。
- [ ] 在具备已登录浏览器会话的环境补充导入 Dialog 桌面端与移动端验收，覆盖搜索、排序、跨页选择和重新预检。

## 2026-08-17 运行时密钥安全查看

- [x] 将部署目标和运行配置集环境变量统一为显式 `public` / `secret` 契约：普通保存只接受 public 项，密钥变更复用事务化安全入口；允许同键两种模式并由 API 分别返回状态，Worker 渲染时 Secret 覆盖普通值。
...
- [x] 新增 AI 模型目录、四类每百万 Token 价格、启停与管理员审计 API；迁移旧默认模型并保留历史 Run 的模型与价格快照。
- [x] 将 AI 模型目录写入收敛到 PostgreSQL 事务级 advisory lock 临界区；首个模型自动启用，并用真实并发测试保证多副本同时启停时至少保留一个启用模型。
- [x] 将模型选择贯通 Web、BFF、Agent Turn/Run、Provider、重试/批准/取消链路和 OpenAPI；禁用模型不影响已有 Run，新请求返回稳定错误码。
- [x] 按普通输入、输出、缓存输入、缓存输出四类用量结算，使用 Run 价格快照、个人钱包归属和幂等记录；旧事件安全跳过，并从通用费率表移除 AI Token 计费项。
- [x] 同步前端模型管理与助手选择器、中英文 i18n、AI/账单公开文档、迁移和契约回归测试；完成 Go、Agent、Web 与 Docs 定向及完整门禁验证。
- [x] 为模型目录增加上下文与单次输出 Token 能力，Run 创建时与四类价格、累计 Token/Credits 上限一起快照，历史 Run 不受管理员后续修改影响。
- [x] 将主回答、工具循环、上下文摘要、标题与下一步预测收敛到统一预算入口；实现按 Run 累计的原子 reservation、过期保守恢复、个人钱包 hold 与 Worker 四价幂等结算。
- [x] Provider 调用前按 Agent/站点上限、模型能力、上下文剩余、Run 剩余与个人钱包可负担额度统一 clamp；不可承受的请求在发往 Provider 前以稳定错误码拒绝。
- [x] 使用独立 `luna_agent_budget_test_*` / `luna_billing_budget_test_*` PostgreSQL 测试库完成并发门禁；测试覆盖双 reservation、钱包普通扣费/负调整、过期恢复锁序、结算幂等与四类价格，清理后管理库无测试前缀残留。
...
- [x] 禁止模型为 Secret 与 Secret Key/Value 字段提供默认值，Web 对历史或恶意默认值强制清空；未输入不提交，生成与清除只使用后端明确动作。
...
## 2026-08-16 AI 应用型 Agent 预算

- [x] 将模型输出、上下文、模型循环、Run 时长、用户输入、工具结果和摘要预算提升到应用型 Agent 档位，并统一 Agent、API 与设置页的默认值和校验范围。
- [x] 保留重试、并发和卡片修复的可靠性边界；公开文档明确高预算是单次请求上限，实际能力仍受模型服务限制。
- [x] 在 AI 助手设置提供从后端配置定义恢复 Agent 运行参数默认值的入口；保留 Provider、Secret、访问范围、可观测和代理配置，并在显式保存后生效。
...
- [x] 将会话 Timeline 改为按完整 Turn 的不透明游标分页，首屏只读取最近记录；批量读取 Run、Item 与事件游标，并保持 SSE 恢复边界一致。
- [x] 会话消息向上滚动时按需加载更早记录并保持阅读位置；会话目录支持继续翻页、标题搜索与稳定排序，同步 BFF、OpenAPI、中英文文档和契约测试。
...
- [x] 统一登录后路由与应用详情的可收缩宽度契约：`PageShell` 与路由动画边界显式使用 `min-width: 0`，部署列表按实际内容容器宽度切换表格与紧凑卡片，横向溢出只保留在 `DataList` 内部滚动区。
...
- [x] 将 Agent 上下文输入预算纳入全局 AI 助手高级运行设置：默认 1024K Token，支持 64–2048K 校验、API 精确换算与 Agent 热更新，并保持长会话压缩策略使用同一预算事实源。
