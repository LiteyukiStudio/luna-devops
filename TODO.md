...
- [ ] 设计平台系统项目空间模型：新增或约定 `platform-system` 系统项目空间，用于承载平台自有探针、采集器和诊断组件；仅平台管理员可见，不参与普通用户账单，不允许普通用户删除或修改；平台管理员可以像维护普通项目空间一样进入查看应用、部署、发布和运行态。
- [ ] 将 Gateway Traffic Probe 从“特殊系统组件安装”迁移为平台系统项目空间下的普通应用：创建 Application、DeploymentTarget、Release 和运行态资源，复用正常部署、日志、Web Console、资源列表和审计链路；系统组件安装记录仅作为应用市场安装入口、状态索引或兼容迁移层。
- [ ] 为平台系统项目空间增加保护规则：禁止普通删除项目空间、应用和部署配置；平台管理员删除/重装探针必须走二次确认；探针相关费用不进入普通用户账单，运行集群维度保留安装状态和最近上报摘要。
...
- [x] 应用市场支持平台组件模板安装：新增系统组件安装记录、独立上报 Token、worker 下发任务和 Kubernetes RBAC/Deployment 生成路径，不创建普通项目空间应用。
