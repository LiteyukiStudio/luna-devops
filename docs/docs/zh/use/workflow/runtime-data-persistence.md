...
导入会把归档直接写入临时 Transfer Pod 挂载的目标卷，不经过对象存储、浏览器预哈希或平台本地完整暂存。API 与 Transfer Pod 会在同一次流传输中独立计算摘要并核对完整性。导入期间会持续复核当前账号、会话或 Token、Scope 和项目角色；任一授权失效都会中止写入。连接中断后不能续传，也不能在原目标卷上重试；请删除失败的导入卷并创建一次全新导入，避免残留数据与新归档混合。如果客户端没有收到创建请求的响应，使用相同 Idempotency-Key 重放只会回读原 Transfer，不会创建或写入第二个目标。只有 Transfer 和数据卷都进入成功/就绪终态后，才可以认为恢复完成。
