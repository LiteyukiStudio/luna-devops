...
Traffic collection is optional and is not installed by default. If Gateway Traffic Probe is not installed, or if it has not successfully reported a time window yet, the billing page shows gateway traffic as unavailable while build, runtime, and storage spend remain available. Platform administrators can install the `Liteyuki Gateway Traffic Probe` platform component from “App Marketplace” and choose the target runtime cluster.
