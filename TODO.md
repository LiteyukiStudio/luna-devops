...
- [x] 保持计费规则修改仅限平台管理员，补齐读取权限、OpenAPI 契约、前后端测试和中英文账单文档。

## 2026-08-14 前端运行时单例依赖门禁

- [x] 统一 CodeMirror `state`/`commands` 依赖实例，修复集群 kubeconfig 编辑器初始化失败，并为 YAML、JSON、文本编辑器增加真实挂载回归测试。
- [x] 增加 React/CodeMirror 单例依赖门禁、编辑器局部文本降级，并同步 AI/人类开发规范、代码健康 SOP 与中英文用户文档。

## 2026-08-14 Agent 交互卡片分段选项样式回归

- [x] 修复分段单选承接整块点击后继承基础圆形控件圆角和焦点环的问题，恢复小圆角卡片外观并保留实例级交互隔离。

## 2026-08-14 Agent 交互卡片渲染隔离
...
## 2026-08-14 系统探针部署配置编辑

- [x] 修复系统组件内部 `sys-*` 部署阶段在 API 与 Web 表单间被错误归一化，导致编辑网络探针配置时误报资源状态冲突的问题。
- [x] 保持不可变阶段由服务端权威回填，并同步 OpenAPI、错误提示、中英文探针配置说明与回归测试。

## 2026-08-14 Agent 单表单卡片末尾呈现

- [x] 为交互卡片增加 `inline` / `turn_end` 渲染位置契约，限制末尾位置只适用于单张、阻塞后续流程的交互表单。
- [x] 保留 SSE 与持久化时间线真实顺序，在前端按轮次投影唯一末尾卡片；多张末尾卡片安全回退原位。
- [x] 同步系统 Prompt、交互 Skill、工具描述、共享类型、恢复与渲染测试。

## 2026-08-13 Agent 大规模工具检索

- [x] 将完整平台工具目录改为按页面与目标下发聚焦子集，并通过内部 `search_tools` 在同一 Run 动态扩展能力。
- [x] 使用 OpenAPI 语义提示、中文适用/禁用/前置/验收描述和场景评测提升工具命中率，补充恢复、防重复检索与可观测指标。
- [x] 同步 Agent 规格、公开使用说明、可观测参考和后续开发约束。

## 2026-08-13 Agent 实际工具集观测

- [x] 每次模型请求前记录实际下发的工具子集，并在 Agent 轮次观测中以胶囊列表呈现。
- [x] 补充公网访问意图识别，确保网关查询与写入工具进入该轮模型工具集。

## 2026-08-13 账单流水说明

- [x] 在账单流水原因下展示具体说明，并隐藏空说明或与标准原因重复的说明。

## 2026-08-13 应用删除与持久化数据生命周期

- [x] 删除应用前实时预览平台托管 PVC，默认保留数据；永久删除需显式选择并确认，且支持先导出。
...
- [x] 将重试次数接入全局 AI 助手设置，完成 API、Agent、前端 Schema、i18n、文档和契约测试同步。
...
- [ ] 按 [应用部署面板后续重构方案](docs-internal/16-application-deployments-panel重构方案.md) 分阶段提取查询、表单、Mutation、实时观察与派生模型；本轮不继续重写。
  - [x] 提取部署配置表单 Hook，隔离默认值、Secret 留空语义、构建环境草稿、运行配置/Hook/端口更新命令及提交载荷，并阻止目标切换或关闭后的旧请求回写。
  - [ ] 后续继续提取发布、运行配置与仓库绑定表单，以及查询、实时观察、派生模型和 Mutation。
...
- [x] 收敛 Agent Run 终态竞态：持久层返回预期、目标与权威当前状态；用户取消与完成写回竞态按取消处理，不再误记为生成失败，并补充稳定可观测字段与回归测试。

- [x] 为 Luna Agent 引入独立 ContextCompiler：按 Token 预算组合系统提示、结构化历史摘要、近期完整轮次和当前工具调用结果；摘要按覆盖水位增量持久化，失败时安全回退且不修改完整权威历史。

- [x] 规范化全局设置保存入口：品牌、注册、安全、计费、数据保留和 AI 配置统一在当前 Tab 操作区保存，移除表单底部重复操作，并修复 AI 保存后仍显示未保存状态的问题。

- [x] 闭环 AI 助手访问范围：默认允许所有已登录且未禁用用户使用，全局设置可切换为仅平台管理员；迁移历史默认值，能力接口和全部会话请求统一执行访问策略，工具权限继续继承当前用户。

- [x] 优化 Agent 部署资料优先级：目标为 Luna DevOps/Kubernetes 且同版本资料同时存在时，以 Kubernetes/Helm 为部署基线、Docker/Compose 为参数补充；资料过期、不完整或不适用时回退到可验证配置。

- [x] 优化 Agent 仓库交付来源决策：用户提供 GitHub、官网或官方文档时，逐服务验证官方 OCI 镜像的来源、版本、架构、可拉取性和运行能力；满足条件时建议镜像直部署，失败或不适用时回退源码构建，并支持混合交付。
...
- [x] 统一看板与应用市场的大屏内容画布：移除概览页过早的内容宽度上限，在超宽屏使用四列资源网格，并保持现有信息密度与响应式布局。
- [x] 扩展 Agent 对话轮详情侧栏到桌面视口三分之二，并统一摘要、时间轴标题和筛选器的响应式内容基线。
- [x] 将 Agent 轮次详情升级为右侧纵向步骤时间轴：从 Tempo 按 Trace ID 读取并归一化 Span，支持模型、工具、异常筛选，默认隐藏外部服务并可按需展示，支持模型与工具内容事件及原始 Span JSON 展开。
- [x] 优化 Agent Span 内容呈现：模型输入输出按角色渲染 Markdown，工具入参与返回使用格式化高亮 JSON，常用 Span 属性支持中英文业务标签，完整字段继续通过原始 Span JSON 提供。
...
- [x] 为 Agent 轮次详情增加版本化诊断 JSON 复制与下载，按真实开始时间稳定组装全量 Span，并保留标准化字段、内容事件和 Tempo 原始数据供 Harness 离线分析。
- [x] 通过浏览器本地偏好按用户记住 Agent 观测时间周期，刷新或重新进入页面时恢复上一次合法选择，异常存储值安全回退到 1 小时。

- [x] 收紧 AI 刷新恢复提示：会话活动时间随用户与助手消息更新，仅在上一会话最后交互不超过 10 分钟时显示 8 秒返回入口。
- [x] 允许在 Agent 工作期间编辑并保留下一条消息草稿，但必须先停止当前任务才能发送，避免并发轮次破坏会话顺序。
- [x] 统一 Agent 资源候选消歧：项目空间、集群、镜像站、账号和凭据等只有一个有效候选时自动采用，多候选、无候选或用户明确更换时才进入选择或创建交互。
- [x] 为 Luna Agent 系统 Prompt 增加可爱的女性猫娘 DevOps 工程师人格与克制、专业的表达风格，不改变工具、权限和安全规则。
- [x] 将 AI `execution_progress` 从模型静态快照改为平台任务绑定：API 提供权威进度快照与 SSE，前端支持实时更新、断线提示、刷新恢复和终态冻结，并禁止模型猜测百分比与步骤状态。
- [x] 优化 AI 助手移动端窗口：从桌面拖拽容器分离，适配 iOS Safari 动态视口、安全区和键盘，并在折叠工具状态中隐藏耗时。
- [x] 修复移动端 AI 助手打开会话列表后候选选项仍悬浮在列表上方的问题。
- [x] 将 AI 会话浏览器重构为移动端独立视图和桌面端可收起侧栏，补齐触控管理入口、空状态与切换动画。
- [x] 优化桌面端 AI 会话浏览器：按助手容器宽度在覆盖抽屉与并排侧栏间切换，避免默认窄窗打开目录后挤压聊天区，并补齐遮罩、Esc、焦点返回和减少动态效果。
- [x] 在桌面端 AI 助手进入宽屏布局时自动展开会话侧栏，并保留用户手动收起能力，提升宽窗口的空间利用率。
- [x] 优化 AI 会话目录的时间层级与行内布局：按当天、当年和跨年自适应显示时间，将运行指示器归入元信息行并收紧操作菜单。
- [x] 统一交互卡片契约与失败修正状态：Agent 作为唯一运行时 Schema 校验方并签发 generationId，前端复用共享类型与权威状态；校验失败按字段路径在原卡片位置有界重试，达到上限后保留错误详情并回灌模型说明。
...
- [x] 逐篇精简中英文公开文档，移除实现算法、内部调用链、界面微交互和过度教学内容；新增常见问题入口承接 Next.js 等专项排障，并保留安装前置、安全警告、兼容范围和不可逆操作说明。
- [x] 更新产品原型为文档式多页面线框。
...
- [x] 统一 write-only Secret 编辑态：已设置的密钥使用 `****** (已设置)` i18n placeholder 提示，表单值保持为空且留空仍表示保留原值。
- [x] 实现 light/dark/system 主题三态。
- [x] 将控制台默认主色调整为 Kubernetes 风格蓝，并同步品牌与原型文档。
- [x] 站点品牌支持 Radix Colors 3.0.0 官方完整品牌色域预设；管理员配置全局默认主题色，用户账号可保持空偏好持续跟随平台或选择个人覆盖色，并按“个人偏好 → 站点默认 → Blue”在 React 渲染前应用，避免默认色闪烁。
- [x] 增加可继承的控制台简约模式：平台管理员配置全局默认状态，账号可跟随平台或显式选择标准/简约；简约模式恢复中性白灰画布，同时保留品牌交互色和状态语义色。
- [x] 降低标准模式大面积主题画布的品牌色浓度：浅色模式向中性白表面混合，深色模式向中性黑表面混合，导航悬停与选中态保留递进层级。
- [x] 统一业务容器边界规则：移除 `Surface`、`Card`、指标组、状态提示、看板快捷卡片和应用市场卡片的外层边框，以实体表面和圆角分层，仅保留内部行、分栏、表单控件与语义结构线。
- [x] 建立 Tailwind v4 Design Token 基础层：集中维护语义颜色、状态色、表面、阴影、圆角、内容间距和响应式页面留白，并让 AppLayout、PageChrome、PageShell、Surface、Card、MetricGroup、Notice 与市场 Banner 优先消费语义 utility。
- [x] 按操作优先级重构平台看板：近期活动与平台准备度形成主工作区，空状态不再维持固定大高度，常用项目空间使用响应式快捷网格，并以真实聚合接口与测试 fixture 分别覆盖空/有数据状态。
- [x] 收敛平台看板综合色噪声：看板与应用概览指标格统一使用中性实体表面，仅在异常图标、关键数值和必要状态徽标保留语义色。
- [x] 修正多色主题侧边栏交互层级：Hover 与选中态改用独立实色 token，确保选中态更深且边缘不与工作区背景混合。
- [x] 为多色主题标准模式增加主色到辅助色的侧边栏选中渐变；单色主题、简约模式与 hover 继续使用实色，并保证渐变两端不与画布背景融合。
- [x] 重绘多色主题选择图标：使用无透明接缝的 SVG 扇区，主色占右下 50%，其余三色均分另一半并保持 45° 分界轴。
- [x] 统一 `DataList` 为轻边界表格风格：表头继承列表实体表面，取消整条灰色色块，仅保留横向结构线；无标题查询工具从左侧排列，有标题时继续与标题左右分布。
- [x] 收紧 `DataList` 外壳留白：为 `Card` 增加显式 padding 变体，列表使用无外壳 padding，由工具区、表头、正文和分页分别维护内部间距，移除表格四周重复的 24px 白边。
- [x] 稳定桌面页面头部高度：`PageChrome` 标题工具行保留统一最小高度，切换到没有工具的 Tab 时 Tab 导航不再上下跳动。
...
- [x] 建立强弱结构线 token：`separator-strong` 用于表头、工具区、分页和固定列边界，`separator-subtle` 用于同类列表行分隔，并同步 DataList 骨架状态。
- [x] 将 `DataList` 内容轨道和横向分隔线按 `spacing-group` 左右内收，避免线条直接碰到列表容器圆角边界，同时不恢复上下外壳留白。
- [x] 收敛 `DataList` 行边界：移除表头上方与末行下方分隔线，圆角 hover 行与相邻分隔线使用同一语义表面色，避免横线穿过悬停色块。
- [x] 建立前端动效 token：统一 fast/standard/slow 时长与 standard/emphasized easing；应用市场分类按钮使用稳定边框盒避免 hover 抖动，模板卡片平滑过渡背景、阴影与轻量位移。
- [x] 统一 `DataList` 表头、数据行和固定操作列的横竖结构线颜色，骨架列表同步使用同一 `separator-subtle` token。
- [x] 加深并再次统一 `DataList` 结构线：表头底线、行间线、固定列竖线与骨架线全部使用 `separator-strong`，消除不同区域的明暗差异。
- [x] 统一 `DataList` 横向分割线的绘制机制：首行与后续行都由数据行顶边绘制同一个 `separator-strong`，消除表头边框合并造成的视觉差异；存在查询、筛选或批量操作工具时在表头上方补充同色内收分割线。
- [x] 建立前端基础布局、路由和 API client。
- [x] 将用户信息和主题切换控件移动到侧边栏底部。
- [x] 将侧边栏账号区退出按钮收敛为账号菜单，集中提供文档、GitHub 仓库与带二次确认的退出入口。
- [x] 新增内容区全局 Topbar，将侧边栏用户信息迁移为右上角头像账号菜单，并为后续消息盒子等全局入口预留扩展空间。
- [x] 将页面标题、桌面工具和页面级 Tab 统一提升到工作区 Topbar；页面通过 WorkspaceChrome Portal 插槽提供导航，正文不再重复占用标题和 Tab 高度。
- [x] 新增账号级消息盒子、待处理请求与实时未读提醒，首批覆盖项目成员变更和项目空间费用归属转移。
...
- [x] 看板读取收敛到专用 `GET /api/v1/dashboard` 聚合接口，统一返回工作摘要、常用项目空间、待关注问题、近期活动和平台准备度；后续看板读模型只扩展该接口，不再由浏览器并发拼装业务列表。
...
- [x] 移动端基础适配：桌面侧边栏在小屏切换为带滑入滑出动画的可折叠侧边抽屉，`DataList` 使用“固有内容宽度 + 容器最小宽度”策略，小屏和窄容器在列宽不足时可左右滑动，同时保留显式配置的右侧固定操作列；`ContentTabs` 小屏改为当前 tab 下拉选择，代码库/镜像站/身份源等配置表单小屏单列化。
...
- [x] 侧边栏导航横条和项目条目统一为语义圆角风格，避免与状态 Badge、分段控制器和胶囊标签混淆。
- [x] 收口控制台设计系统：普通命令控件使用标准圆角，状态与模式组件保留胶囊形；看板减少卡片嵌套，项目空间列表优化筛选、排序、操作层级和移动端密度。
- [x] 完成前端视觉整改阶段 A：补齐表面、状态与阴影语义 token，新增 `PageShell`、`PageToolbar`、`Surface`、`Section`、`MetricGroup` 和 `Notice`，并同步中英文开发文档与长期规范。
- [x] 完成前端视觉整改阶段 B 样板页：项目空间列表、平台看板和全局设置分别验证列表、概览和设置页模板；保持原有查询、权限、校验和提交行为不变。
- [x] 完成前端视觉体系后续整改阶段 A：结构化加载 skeleton 覆盖应用初始化、列表、概览、设置、模板与工具工作区；侧边栏使用可滚动内容与稳定 Footer；零数据列表隐藏分页；开发悬浮面板具备安全边距、浮层避让和持久隐藏能力。
- [x] 完成前端视觉体系后续整改阶段 B：事件移动端筛选进入 Sheet，项目空间移动端工具栏与保留列收口，`MetricGroup` 支持风险语义表面，移动端高频列表减少无意义横向滚动。
- [x] 完成前端视觉体系后续整改阶段 C 核心页面：应用概览收敛部署引导、健康指标和行列表层级；账号表单限制设置宽度；用户认证状态减少 Badge；运营面板增加骨架、超时失败、重试与新窗口入口；公共空状态区分首次配置和筛选为空。
- [x] 完成前端视觉体系后续整改阶段 D 词汇收口：统一 Git Provider、OIDC Provider、权限范围（Scopes）、仓库所有者（Owner）等技术词汇，并将命名规则写入中英文视觉规范。
- [x] 统一设置表单操作区：新增 `FormActions`，桌面端提交按钮恢复内容宽度并右对齐，移动端保持全宽；准入策略、账号资料、密码和注册设置首批接入，表单正文限制舒适阅读宽度并按业务分组。
- [x] 统一全局设置各 tab 的保存位置：品牌、安全、计费和数据保留从 tab 顶栏迁移到当前表单底部，与注册与登录保持一致；构建配置继续使用独立编辑流程。
- [x] 将低边界主题色工作区应用到整个登录后控制台：所有页面的顶栏与内容画布使用品牌主题浅色背景，公共 `Surface` 和 `Card` 以实体表面、圆角与轻阴影替代重复描边，并保持深色模式与品牌色自动适配。
- [x] 将侧边栏融入全局主题画布：应用根节点提供统一主题背景，桌面导航透明继承且移除右侧分隔边，移动抽屉保留实体主题表面，导航状态统一使用主题语义色。
- [x] 移除侧边栏菜单类型之间的分割线，改用分组标题与纵向留白维持导航层级，进一步弱化侧边栏的框架感。
- [x] 调整应用市场 Banner 与全局主题画布的层级：Banner 使用无渐变、无光晕的纯净实体抬升表面，搜索框使用内嵌表面，避免与页面背景融为一体。
- [x] 降低侧边栏菜单分类名称的视觉权重：分组标题缩小为 11px、使用正常字重和更低对比度，菜单项字号保持不变。
- [x] 统一登录后内容画布的响应式内边距：水平留白保持移动端 32px、中屏 48px、桌面端 64px，垂直留白收紧为 16px、24px、32px；顶栏标题与正文使用相同横向基线。
- [x] 将桌面端页面标题从独立全宽 topbar 下沉到内容工作区，与正文 Tab 和工具栏共享左侧基线及紧凑纵向节奏；移动端继续保留菜单标题栏。
- [x] 重构页面头部职责：桌面端标题与页面工具左右分布，`ContentTabs` 独立为纯导航组件；现有 Tab 工具和 `PageHeader` 操作统一通过公共工具槽提升，中小屏回落到正文操作区。
- [x] 将标题、工具和可选 Tab 收敛为统一 `PageChrome`：标题/工具固定在第一行，Tab 仅在页面传入时单独显示第二行；移除过渡期拆分的 page-tools 组件与上下文。
- [x] 简化公共 `DataList` 表头：直接移除表头底线及固定操作列表头分隔线，使灰色表头成为无边框连续表面；正文列边界保持可辨识。
- [x] 重构公共表格边界：`TableFrame` 改用单层原生语义边框与圆角裁剪，移除嵌套背景模拟边框及表头无效模糊合成层，消除圆角处白色遮罩接缝。
- [x] 校正侧栏品牌区留白：品牌区高度调整为 72px，40px Logo 使用 16px 顶部与左侧等距留白。
- [x] 微调公共内容 Tab 指示线：灰色基线与主题色高亮线使用同一底部坐标，高亮线及基线使用圆角端点，移除触发器自身重复下边框。
- [x] 移除公共 `DataList` 外边框，保留圆角、表面底色、行分隔和分页结构，降低列表外壳的框线感。
- [x] 统一项目空间与用户管理列表结构：创建操作提升到标题右侧，搜索、范围和排序控件收进 `DataList` 表头工具区；公共列表新增 `toolbar` 插槽并移除仅项目空间使用的 `PageToolbar`。
- [x] 精简列表工具区信息层级：用户、项目空间、事件、访问令牌和开发者应用移除与页面上下文重复的“XX 列表”标题，搜索、筛选、排序和刷新控件统一从左侧开始排列。
- [x] 重构公共列表为单层表格结构：`TableFrame` 统一持有可滚动表体与可选表尾，`DataList` 将分页纳入表尾并移除无业务意义的 Card 外壳，纯列表页面直接展示完整表格。
- [x] 收口内容区对齐与表面层级：`PageShell` 移除自动居中外边距和额外水平留白，所有页面正文与顶栏标题共享布局 padding 基线；普通 `Card`、`Surface` 和市场 Banner 移除常驻阴影。
- [x] 按内容密度统一面板内边距比例：主表单、设置和账号面板使用 24px，目录与指标卡片保留 16–20px，列表、日志、拓扑、终端和 iframe 外壳保持无外层 padding。
...
- [x] 将搜索选择器扩展为统一的单选/多选基础组件，项目空间、用户和项目成员候选复用同一交互；固定少量枚举继续使用 shadcn Select/NativeSelect。
...
- [x] 补齐运行集群管理列表分页，并完善镜像站、全部镜像凭据与运行集群的前端翻页；镜像记录列表使用受约束的视口高度，修复记录较多时无法纵向滚动。
...
- [x] 规范应用市场品牌图标来源：优先使用项目官方 SVG/图片，其次使用可追溯的品牌图标库，找不到可信资产时才使用中性占位图；成熟产品不再绘制近似 Logo，并将外部资产随前端静态资源托管、记录来源与许可。
- [x] 重设计应用市场浏览体验：使用市场导语、快捷分类、搜索排序和轻量模板陈列建立发现层级；模板卡片优先说明用途，端口、资源、镜像与来源降为辅助信息，并降低重复安装按钮的视觉噪声。
- [x] 完成应用市场第二轮视觉重构：搜索进入首屏主视觉，所有模板统一进入可筛选和排序的目录；进一步减少容器、状态标签、参数底色和按钮边框，卡片移除资源规格与镜像地址并收口到安装弹窗，强化品牌、用途与内容节奏。
...
- [x] 支持平台构建模板：部署配置可选择“仓库 Dockerfile”或“平台模板”，内置 Go、Node.js、Next.js standalone、Bun、Python + uv、Rust、Ruby、Java Maven、Java Gradle、.NET 和静态站点模板及运行参数；Next.js 模板遵循官方容器示例，按 npm/pnpm/Yarn lockfile 安装和构建、使用三阶段镜像并以非 root 用户运行；模板选择器统一展示本地托管的官方技术标志与名称，仓库文件探测会优先推荐匹配模板；Kubernetes 构建 Job 克隆仓库后，通过只读 Secret 挂载平台渲染的临时 Dockerfile，并按部署配置中的构建上下文执行 BuildKit 构建；仓库没有 Dockerfile 时前端主动推荐模板，用户显式选择模板时则覆盖仓库原有 Dockerfile；BuildRun 保存模板版本、参数、渲染结果和校验和快照。
...
- [x] 部署配置编辑表单把运行资源、项目空间公共配置和部署级覆盖合并为“运行配置”，紧跟构建设置展示；顶层区块的长说明改为可 hover/focus 的提示，减少展开后的常驻说明文本。
...
- [x] 构建环境扩展为全局、项目空间、应用和部署配置四级，按部署配置 > 应用 > 项目空间 > 全局覆盖；BuildRun 固化变量与密钥引用快照，前端复用统一键值编辑器。
...
- [x] 将 AI Provider 返回的输入/输出 Token 纳入用户钱包计费：按 `model.completed` 权威事件异步补结算，默认费率分别为每千 Token 1/4 credits，可在站点计费配置中调整，并以事件 ID 保证幂等。
...
- [x] 修复通知重复投递和 Compose 直达链接：同一事件每个渠道最多生成一条投递，全局/项目规则重叠不再重复发送；Docker Compose Worker 同步注入 `PUBLIC_BASE_URL`。
- [x] 修复 Webhook 成功后重复发送：投递成功状态不再向 `notification_deliveries` 写入渠道专属的 `last_delivered_at` 字段，避免落盘失败触发 5 次队列重试。

## 14. 事件中心

目标：把构建、发布、Hook、访问入口和证书等业务变化沉淀为统一事件。事件先落盘，再供事件页面和通知规则共同消费；通知投递、审计日志、Worker 任务轨迹和 Kubernetes Events 继续保持各自职责。详细方案见 `docs/docs/zh/developer/event-center-design.md`。

- [x] 新增 `PlatformEvent` 模型、迁移、事件目录和统一写入服务；事件使用稳定类型、分类、严重级别、资源归属、结构化详情、关联 ID 和详情链接。
- [x] 调整通知入口：业务事件先写入 `PlatformEvent`，再匹配通知规则并异步投递；没有通知规则或渠道时仍保留完整事件历史。
- [x] 接入首批构建、发布、Hook、访问入口和证书状态变化事件，并按状态变化去重，避免 Worker 重试和周期同步重复写入。
- [x] 提供事件列表、详情和事件目录 API，支持分页、时间、项目空间、应用、部署配置、分类、类型、严重级别和结果筛选。
- [x] 事件筛选支持项目空间、应用、部署配置、分类、类型、严重级别和结果多选；后端兼容原单值参数并支持复数集合查询。
...
- [x] 事件页面默认聚焦警告和错误；构建失败事件提供包含 BuildRun ID 的现场链接，并在构建页自动定位和高亮对应记录。
...
- [x] 将下一步候选从消息气泡抽离为悬浮在消息区底部的单行胶囊栏：不绘制独立分隔线，消息列表预留可滚动底部空间；新会话按页面提供 3～5 个 i18n 预设，首轮后只展示最新 `create_options`，支持 primary/default/danger 语义样式、单项幂等状态、横向滚动及 reduced-motion 兼容的右侧弹簧切换。
...
- [x] 将 Agent 长会话上下文重构为 Token 高低水位驱动的持久压缩：完整 Timeline 保持权威，结构化摘要按连续 Turn 水位增量追赶，近期原文与当前工具调用成对保留；压缩失败安全降级且模型请求始终受输入预算约束。
- [x] 将 Agent 上下文输入预算纳入全局 AI 助手高级运行设置：默认 256K Token，支持 64–1024K 校验、API 精确换算与 Agent 热更新，并保持长会话压缩策略使用同一预算事实源。
- [x] 将 Agent 内部分类写死的运行时参数纳入全局 AI 助手“高级设置”：上下文与压缩（压缩触发/目标比例、保留近期轮次、未压缩阈值、单次压缩最大轮次、摘要输入/输出预算、历史工具结果预算）、模型与执行（回复最大输出 Token、Run 最大模型轮次、输入大小上限、跳转动作有效期）、工具结果与卡片（工具结果上下文预算、交互卡片修复上限）共 15 项；每项提供默认值与范围校验，经受认证内部配置动态下发并由 Agent 热更新，中英文文档与配置说明同步。
- [x] 扩展下一步建议的统一视觉契约：支持可选 Emoji、受控项目图标和可信 HTTPS 图片，整组必须全量采用同一视觉类型或全部回退纯文本，并用固定视觉槽保持胶囊布局稳定。
