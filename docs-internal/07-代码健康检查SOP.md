...
固定基线提交、目标环境和审计范围，覆盖认证入口、会话与 Token、OIDC、权限、
Secret、审计、长连接、导出、迁移、OpenAPI、前端类型、依赖、CI、镜像和 Helm。

| 级别 | 判定 | 退出条件 |
| --- | --- | --- |
| P0 | 可绕过认证/授权、泄露 Secret、跨租户读写、重放凭据、破坏数据或接管发布 | 立即阻断合并和发布；修复并由独立复审通过 |
...
- 生产数据库的建表、改列、索引、约束、数据回填和初始系统数据只允许通过 `migrations/`
  下的编号 SQL 迁移交付；API/Worker 启动时只负责真实连接检查和执行 `golang-migrate`。
- 禁止在生产启动路径使用 GORM `AutoMigrate`、临时 DDL、重复 backfill 或启动期 seed 修补
  Schema。GORM 模型用于运行时映射，不是第二套迁移来源。
- 不得通过 `migrate.Force` 自动认领无版本历史的旧库或修复 dirty 状态。dirty、迁移缺口和
  非空无版本数据库必须 fail closed，由部署管理员确认备份后使用显式迁移工具处理。
- 新增或修改持久模型时，同一事项必须包含正向/反向（或明确不可逆）迁移；可销毁 PostgreSQL
  测试必须从空库完整迁移两次，并检查最新版本、dirty 状态、模型表列覆盖和关键约束。
- 数据修复只执行一次并绑定迁移版本。需要长期保证的业务不变量必须在 Service、约束或事务中
  实现，不得依赖每次进程启动时重新扫描全表。
- 项目首次正式发版前允许在明确的数据重建边界内压平迁移历史。本仓库以
